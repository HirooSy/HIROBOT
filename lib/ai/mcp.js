...
//
// Sumber klasifikasi SEKARANG MURNI SATU LAPIS — FLOOR sistem saja
// (per keputusan owner: plugin gak lagi dipercaya buat self-declare risk-nya
// sendiri lewat handler.ai.risk, soalnya plugin ceroboh/nakal bisa nge-declare
// dirinya "low" padahal harusnya bahaya). FLOOR dihitung otomatis dari flag
// `handler.owner` / `handler.rowner` plugin plus pattern nama+tag+command-nya
// (RISK_*_PATTERNS di bawah), TIDAK BISA diturunkan atau dinaikkan oleh
// plugin manapun.
//
// Satu-satunya sisa pengaruh dari `handler.ai` adalah `ai.isTool === false`
// (plugin eksplisit MATIIN dirinya dari AI) — ini tetap dipercaya karena cuma
// bisa MEMPERKETAT (jadi blocked), gak pernah melonggarkan keamanan.
// `handler.ai.risk` / `handler.ai.description` SUDAH TIDAK DIBACA SAMA
// SEKALI lagi.
...
export function classifyPluginRisk(name, plugin) {
    ...
    // PENTING: ai.isTool BUKAN gate blocked/tidak-blocked untuk jalur run_plugin
    // ini. isTool cuma sinyal terpisah soal APAKAH plugin ini JUGA di-load
    // sebagai standalone MCP tool langsung (lihat buildDirectPluginTools()) --
    // isTool:false tetap sepenuhnya bisa dijalankan lewat run_plugin, cuma gak
    // dapat "jalur pintas" tool langsungnya. Risk classification di sini
    // SELALU berlaku ke keduanya, apapun nilai isTool-nya.
...
- Plugin command mengikuti struktur handler.js:
  ...
  handler.ai = {
    ...
    isTool: true,       // true = boleh dipanggil AI lewat run_plugin. Set false untuk
                        // eksplisit menonaktifkan plugin ini dari AI sama sekali
                        // (selalu jadi blocked, apapun risk-nya).
