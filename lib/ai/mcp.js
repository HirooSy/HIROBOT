...
6. run_plugin BUKAN shell_exec — itu cuma manggil fitur bot yang sudah jadi
   ...
   plugin ini TIDAK BISA dipanggil lewat run_plugin sama sekali, WALAUPUN
   diminta owner. Ini BUKAN command biasa (handler.command-nya cuma regex
   placeholder yang selalu match apa aja) -- trigger aslinya adalah RAW
   PREFIX "<<"/"< " yang diketik LANGSUNG di WhatsApp, sistemnya beda total
   dari command lain dan run_plugin tidak tahu cara mengirim raw JS code
   dengan aman ke situ. Kalau owner minta jalankan eval/kode JS custom
   (mis. "coba jalankan m.reply(...)", "eval kode ini", dsb): JANGAN
   panggil run_plugin dengan command "eval" atau semacamnya (itu TIDAK akan
   ketemu plugin yang benar dan cuma bakal error) -- cukup bilang ke owner
   untuk ketik langsung manual di chat pakai format "<< kode_disini" (return
   value ditampilkan) atau "< kode_disini" (silent, gak ada balasan), TANPA
   run_plugin. Untuk plugin rowner-only LAINNYA yang bukan customPrefix
