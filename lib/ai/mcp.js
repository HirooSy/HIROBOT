...
// Ambil contextInfo dari message apapun tipenya (extendedTextMessage, imageMessage,
// videoMessage, documentMessage, stickerMessage, dll) — bukan cuma extendedTextMessage.
// Ini penting karena kalau user reply sambil kirim media (foto+caption misalnya),
// quotedMessage-nya nyempil di imageMessage.contextInfo, bukan extendedTextMessage.contextInfo.
//
// PENTING: pakai getContentType (sama seperti yang dipakai lib/simple.js untuk getter m.quoted)
// alih-alih Object.keys() manual — field di object Message dari baileys/protobuf itu
// non-enumerable, jadi Object.keys() bisa gagal nemuin key yang sebenarnya ada.
...
export function getContextInfo(m, override) {
    ...
    const msg = m?.message;
    if (!msg) return null;
    // Coba pakai m.quoted punya library (lib/simple.js) kalau ada — itu sudah pasti benar
    // karena dipakai juga oleh fitur reply/quote bawaan bot.
    if (m.msg && m.msg.contextInfo) {
        return m.msg.contextInfo;
    }
    // Fallback manual: cek key yang umum dulu (lebih cepat dari import async),
    // baru broad scan kalau belum ketemu.
...
14. GITPUSH / COMMIT KE GITHUB — kalau owner (rowner) minta push perubahan project ke
    ...
    sama, tanpa nunggu diminta ulang per langkah:
      ...
      b. Untuk TIAP file yang berubah, pakai shell_exec "git diff -- <file>" (file yang
         ...
         benar-benar baca lewat git diff/read_file di langkah ini.
      c. Update CHANGELOG.md (root project, read_file dulu untuk lihat isi/format entri
         ...
         - Tiap file diberi SATU baris status (+ Add / - Delete / * Edit) diikuti diff
           ringkas (~15 baris, baris "+"/"-") KHUSUS untuk status Edit — file Add/Delete
           tidak perlu diff di bawahnya, lihat contoh.
...
\`\`\`diff\`\`\` tunggal berisi bullet ringkasan lalu separator lalu status per file. SEKALI
LAGI: nama file "lib/scraper/test.js" dan isi diff di bawah ini CUMA CONTOH PERAGA FORMAT —
di entri sungguhan, ganti dengan file dan diff yang BENAR-BENAR berubah hasil git diff turn
ini, JANGAN disalin apa adanya):
...
+ const worker = new Worker(scriptPath, { workerData: { resourcesPath } });
+ worker.on("error", (err) => console.error("[VOIP] worker error:", err));
\`\`\`
</sub>
