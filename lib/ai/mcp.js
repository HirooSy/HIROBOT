...
// Satu-satunya tempat resolusi command -> plugin object. Dipakai internal
// oleh execPluginCommand, DAN diexport supaya caller lain (mis. run_plugin
// di plugin.js) bisa cek properti plugin (mis. handler.ai.summarize)
// SEBELUM eksekusi tanpa perlu duplikat logic matching command sendiri
// dengan cara yang gampang out-of-sync/salah gara-gara duplikasi.
export async function resolvePlugin(command) {
    ...
    for (const [name, plugin] of Object.entries(plugins || {})) {
        ...
        // Plugin yang pakai handler.customPrefix (mis. eval.js dengan
        // customPrefix /^<?< / buat trigger "<"/"<<") biasanya declare
        // handler.command sebagai regex catch-all (mis. /(?:)/i yang match
        // string APAPUN termasuk kosong) karena matching sebenarnya terjadi
        // lewat customPrefix parsing di message handler, BUKAN lewat nama
        // command biasa. Kalau ikut di-resolve di sini, plugin ini bisa
        // "mencuri" match buat command APAPUN yang gak dikenali plugin lain
        // -- exclude eksplisit, customPrefix plugin HARUS dipanggil lewat
        // jalur terpisah yang sadar soal syntax prefix-nya (lihat
        // resolveCustomPrefixPlugin), bukan lewat resolver nama-command ini.
...
// Resolver TERPISAH khusus buat plugin customPrefix (mis. eval.js). Dipanggil
// eksplisit dari run_plugin ketika command yang diminta memang berupa raw
// prefix syntax (mis. "<< kode" atau "< kode"), BUKAN nama command biasa.
export async function resolveCustomPrefixPlugin(rawInput) {
    const { plugins } = await import('../plugins.js')
    for (const [name, plugin] of Object.entries(plugins || {})) {
        if (!plugin || typeof plugin !== 'function' || !plugin.customPrefix) continue
...
// Eksekusi eval.js LANGSUNG lewat AI, terpisah total dari execPluginCommand/
// run_plugin biasa -- eval.js itu customPrefix plugin (trigger "<<"/"< "),
// bukan command-name biasa, jadi gak lewat resolvePlugin sama sekali.
// SENGAJA gak ada tool generik run_plugin yang bisa nyentuh ini (lihat
// exclude customPrefix di resolvePlugin) -- ini satu-satunya jalur AI bisa
// trigger eval, dan SENGAJA dibuat fungsi terpisah/eksplisit (bukan folded
// ke run_plugin) supaya gatingnya jelas & gampang diaudit di satu tempat.
//
// PENTING (diminta eksplisit oleh owner, TANPA confirmation step): fungsi
// ini TIDAK minta konfirmasi user sebelum eksekusi, beda dari command
// risk:medium/none lainnya. Gate SATU-SATUNYA adalah _currentIsROwner
// (real owner bot) -- non-owner selalu ditolak sebelum kode apapun
// dieksekusi, gak ada jalan lain buat lolos gate ini.
...
function stripInternalNotes(str) {
    ...
    return str
        ...
        // Kadang model ikut nge-echo balik status internal run_plugin
        // ("Command "X" selesai dijalankan (risiko: ...). Plugin sudah
        // mengirim balasannya sendiri...") padahal itu instruksi buat
        // model doang, bukan buat ditampilkan ke user -- buang kalimat
        // itu di manapun dia muncul di teks akhir.
        ...
        // Varian yang sama tapi dari run_eval (wording beda dikit dari
        // run_plugin: "Kode eval sudah dijalankan..." bukan "Command X
        // selesai dijalankan..."), buang juga kalau di-echo balik oleh model.
...
async function mcpLoop(history, apiKey, model) {
    ...
    for (let i = 0; i < MAX_LOOPS; i++) {
        ...
        if (functionCalls.length === 0) {
            ...
            // Backstop kode buat pola fabrikasi yang TERBUKTI kejadian berkali-kali
            // walau sudah dilarang keras di prompt (rule 6b): lastToolOutput bilang
            // command SUKSES ("selesai dijalankan ... Plugin sudah mengirim
            // balasannya sendiri") tapi model malah menulis balasan yang mengaku
            // gagal/error/kendala teknis. Ini kontradiksi yang gak masuk akal --
            // satu-satunya penjelasan adalah model mengarang, jadi buang balasan
            // itu daripada diteruskan ke user sebagai info palsu.
        ...
        const responseParts = []
        // Dedup pemanggilan tool yang PERSIS SAMA (nama + args identik) dalam
        // SATU response model yang sama. Ini pernah kejadian nyata: model
        // balikin 2 functionCall run_plugin({command:"menu"}) sekaligus di
        // response yang sama, dan tanpa dedup ini keduanya BENERAN dieksekusi
        // -- akibatnya menu (atau command lain yang side-effecting, kirim
        // pesan WA dsb) kekirim dobel ke user. Gak ada skenario valid di mana
        // model SENGAJA manggil tool yang SAMA PERSIS 2x sekaligus di 1
        // response yang sama, jadi aman untuk selalu dedup di titik ini.
        //
        // PENTING: Gemini API mewajibkan tiap functionCall dalam 1 model turn
        // punya functionResponse yang match di turn berikutnya (posisinya
        // dicocokkan by name, bukan by index, tapi tetap harus ADA) -- jadi
        // duplicate yang di-skip TETAP harus dapat functionResponse sintetis,
        // JANGAN cuma di-omit gitu aja atau request berikutnya bisa error.
