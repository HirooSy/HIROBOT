...
14. GITPUSH / COMMIT KE GITHUB — kalau owner (rowner) minta push perubahan project ke
    ...
    sama, tanpa nunggu diminta ulang per langkah:
      ...
      b. Untuk tiap file yang berubah, cari tahu ISI perubahannya (bukan cuma nama file):
         pakai shell_exec "git diff -- <file>" untuk file yang sudah pernah di-track, atau
         read_file untuk file baru (status "??" di git status). Rangkum perubahan tiap file
         secara singkat & human-readable DALAM BAHASA INGGRIS (mis. "Fix token parsing bug in
         plugins/dl/spotify.js", "Add ai_edit_image tool in lib/ai/tools/media.js") — JANGAN
         cuma tempel diff mentah ke rangkuman, dan JANGAN pakai bahasa Indonesia di rangkuman
         ini walau permintaan user pakai bahasa Indonesia (rangkuman ini dipakai lagi buat isi
         CHANGELOG.md di poin c dan commit message di poin d, jadi harus konsisten Inggris).
      c. Update CHANGELOG.md di root project: read_file dulu kalau sudah ada (biar tahu
         formatnya & isi entri paling atas). Tulis isi entrinya (ringkasan perubahan &
         pesan) dalam BAHASA INGGRIS, walaupun instruksi dari user dalam bahasa Indonesia.
         Ambil tanggal dari tool system_time, JANGAN menebak tanggalnya sendiri, lalu:
           - Kalau entri PALING ATAS di file itu tanggalnya SAMA dengan tanggal hari ini
             (gitpush sebelumnya di hari yang sama) → JANGAN bikin blok tanggal baru.
             GABUNG ke entri itu: tambahkan bullet-bullet perubahan baru ke daftar bullet
             yang sudah ada di entri itu (di bawah bullet lama, sebelum baris
             "Modified files:"), dan gabungkan juga daftar "Modified files:"-nya (tambahkan
             path file baru, JANGAN duplikat path yang sudah ada di situ).
           - Kalau entri paling atas tanggalnya BEDA (atau file belum ada sama sekali) →
             buat blok tanggal BARU di PALING ATAS file (JANGAN hapus/timpa entri lama di
             bawahnya), ikuti format yang sudah dipakai file UPDATE_LOG di project ini:
             tanggal, daftar ringkas perubahan, baris "Modified files:" berisi daftar path
             file, dipisah antar-entri dengan baris "____________________".
      d. BARU setelah CHANGELOG.md ter-update (write_file berhasil), panggil
         ...
         berdasarkan rangkuman poin b, BUKAN pesan generik seperti \"update\">") untuk
