...
const DOWNLOAD_PLATFORM_KEYS = ['tiktok', 'instagram', 'youtube', 'youtube_audio', 'twitter']
...
},
{
    ...
    execute: async ({ url }) => {
        ...
        if (ctx().conn && ctx().currentJid) {
            await ctx().conn.sendMessage(ctx().currentJid, { video: { url: result.videoUrl }, caption: `*${title}*\n_via ${process.env.BOT_NAME}_`, mimetype: 'video/mp4' }, { quoted: ctx().currentM })
