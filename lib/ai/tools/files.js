import { ctx, buildSimpleDiff, readFileToolCore, appendAutoHealStatus } from '../mcp.js';
import fs from 'fs';
...
export default [
        ...
        execute: async ({ file_path, content }) => {
            ...
            if (isEsmModule) {
                ...
                if (requireMatch) {
                    ...
                    return `REJECTED: ${file_path} masih pakai require() di baris ~${lineNum} — project ini ESM murni ` +
                        `(lihat CODE_CONVENTIONS), semua import HARUS pakai "import x from 'y'" di bagian atas file, ` +
                        `atau "const x = (await import('y')).default" kalau butuh dynamic import (mis. di dalam fungsi, ` +
                        `atau kalau importnya kondisional). Perbaiki dulu jadi import ESM, baru panggil write_file lagi. ` +
                        `write_file TIDAK menulis apapun ke disk selama masih ada require() di file .js/.mjs.`;
                }
            }
            ...
            if (existed && oldContent.length > 200) {
                ...
                if (shrinkRatio > 0.2) {
                    ...
                    if (ctx().autoHealActive && ctx().autoHealNotifyJid && ctx().conn) {
                        await appendAutoHealStatus(`*Auto-Heal DIBATALKAN* untuk ${file_path}\n\n` +
    ...
    },
    {
        ...
        execute: async ({ query, folder = '.' }) => {
            ...
            walk(target);
            if (!results.length)
                return `Tidak ada file cocok dengan "${query}"`;
    ...
    },
    {
        ...
        execute: async ({ file_path, content, caption }) => {
            if (ctx().autoHealActive) {
                return 'Tool ini nonaktif selama auto-heal — diff perubahan sudah otomatis ditampilkan ke owner lewat write_file, jangan kirim file lagi di sini.';
            }
            ...
            if (!finalContent) {
                if (!ctx().isOwner)
                    return 'Content kosong (mode baca-dari-disk) cuma boleh untuk owner. Isi parameter content manual dengan konten yang mau dikirim.';
                const abs = path.resolve(ROOT, file_path);
                if (!fs.existsSync(abs))
                    return `Content kosong dan file tidak ditemukan di disk: ${file_path}. Isi parameter content manual, atau pastikan file_path benar (boleh path absolut).`;
                if (fs.statSync(abs).isDirectory())
                    return `${file_path} adalah folder, bukan file. Kasih path ke file-nya.`;
    ...
    },
    {
        ...
        execute: async ({ file_path, title, description }) => {
            if (ctx().autoHealActive) {
                return 'Tool ini nonaktif selama auto-heal — diff perubahan sudah otomatis ditampilkan ke owner lewat write_file, jangan kirim codeblock lagi di sini.';
            }
            if (!ctx().conn || !ctx().currentJid)
                return 'WA connection not ready';
            if (!file_path)
            ...
            if (fs.statSync(abs).isDirectory())
                return `${file_path} adalah folder, bukan file.`;
