...
// ── Local temporary file upload storage ─────────────────────────────
// Files POSTed to /api/upload are stored on disk under data/upload with
// a random 7-char name and served back at /files/<name>. Anything older
// than 7 days is purged automatically.
const UPLOAD_DIR = path.join(process.cwd(), 'data', 'upload')
const UPLOAD_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000 // 7 days
const UPLOAD_MAX_BODY_BYTES = 100 * 1024 * 1024 // 100MB
...
ensureUploadDir()
cleanupOldUploads()
setInterval(cleanupOldUploads, 60 * 60 * 1000).unref() // hourly sweep
...
// ── Interactive HTML action tokens ──────────────────────────────────
// When a plugin builds an interactive aiRich HTML card (e.g. a "Download"
// button), it registers a `run` function here (chatId is captured in the
// closure, not sent over the wire) and gets back a random token. The HTML
// only ever carries the token + whatever small args it needs (an index, a
// page number, ...) - it never carries the chatId or any server-callable
// logic itself, so the HTML can't be used to make the bot do something in
// an arbitrary chat or run arbitrary code.
//
// This replaces the older per-feature `action: 'sendFile' | 'sendScraperPost'
// | ...` string dispatch: server.js no longer needs to know what any given
// plugin's button actually does. A plugin just does:
//
//   const token = global.registerHtmlAction({
//     chatId: m.chat,
//     run: async (conn, chatId, args) => { ...whatever... },
//     singleUse: false
//   })
//
// and the button's WS/HTTP call becomes: { type: 'aiRichAction', token, ...anyArgs }
...
async function performHtmlAction(conn, token, args = {}) {
  ...
  try {
    ...
    // A run() can return nothing (implicit success), a plain result object
    // to hand straight back to the HTML ({ success, message, ...anyData }),
    // or throw to signal failure.
...
function buildRoutes(conn) {
  ...
  // Generic on-demand image proxy: lets any HTML gallery embed <img
  // src="/api/proxy-image?url=..."> for a thumbnail hosted on an external
  // CDN that the WebView can't fetch directly (referer checks, CSP, etc).
  // The server fetches it (a plain server-to-server request, unaffected by
  // browser-side referer/CORS rules) and streams it back from the bot's
  // own trusted origin. Whitelisted by hostname so this can't be turned
  ...
  get('/api/proxy-image', async (req, res) => {
    try {
      ...
      // Upstream CDN connections from this server have been intermittently
      // slow/flaky (occasional ETIMEDOUT), so retry once with a short
      // per-attempt timeout rather than one long attempt - keeps the total
      // wait well under any reverse-proxy's own timeout (avoiding 522s)
      // while still giving flaky connections a second chance.
      let upstream
      let lastErr
...
// Exposed so plugins (e.g. login.js) can drive the same auth flow this
// dashboard's /api/login, /api/register, /api/verify-otp, /api/resend-otp
// routes use, but over the aiRichAction websocket instead of fetch() -
// WhatsApp's HTML sandbox appears to allow the websocket but block plain
// cross-origin fetch()/XHR calls, even with CORS headers present.
global.authHelpers = {
  sendOtp,
  verifyOtp,
  hashPassword,
  ...
  if (!_server) {
    ...
    _server = global.server = createServer(async (req, res) => {
      try {
        ...
        // These endpoints are meant to be called from inside WhatsApp's own
        // sandboxed HTML webview, which is a different origin than this
        // server - so they need explicit CORS headers, unlike the rest of
        // this dashboard (which relies on same-origin/cookie auth and
        // should NOT get blanket CORS enabled).
