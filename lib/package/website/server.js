  ...
  if (!_server) {
    ...
    _server = global.server = createServer(async (req, res) => {
      try {
        ...
        // This one endpoint is meant to be called from inside WhatsApp's
        // own sandboxed HTML webview, which is a different origin than this
        // server - so it needs explicit CORS headers, unlike the rest of
        // this dashboard (which relies on same-origin/cookie auth and
        // should NOT get blanket CORS enabled).
        if (pathname === '/api/aiRich/action') {
