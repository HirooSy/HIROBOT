...
// button), it registers the actual chatId + action + payload here and gets
// back a random, single-use token. Only that token is embedded in the HTML
// - never the raw chatId - so the HTML can't be used to make the bot send
// things into an arbitrary chat.
const _pendingHtmlActions = new Map()
const HTML_ACTION_TTL_MS = 15 * 60 * 1000

function registerHtmlAction({ chatId, action, payload, singleUse = true }, ttlMs = HTML_ACTION_TTL_MS) {
  const token = randomBytes(16).toString('hex')
  _pendingHtmlActions.set(token, { chatId, action, payload, singleUse, expiresAt: Date.now() + ttlMs })
  return token
}
...
async function performHtmlAction(conn, token, index) {
  ...
  if (rec.action === 'sendFile') {
    const { url, urls, types, filename, caption } = rec.payload || {}
    const i = Number(index) || 0
    const target = Array.isArray(urls) ? urls[i] : url
    if (!target) return { success: false, message: 'Nothing to send.' }
    const itemType = Array.isArray(types) ? types[i] : null
    const resolvedFilename = filename || (itemType === 'video' ? 'pinterest.mp4' : 'pinterest.jpg')
    await conn.sendFile(rec.chatId, target, resolvedFilename, caption || '', null)
    return { success: true, message: 'Sent to chat.' }
  }
...
function buildRoutes(conn) {
  ...
  post('/api/aiRich/action', async (req, res, ctx) => {
    try {
      const { token, index } = ctx.body
      if (!token || typeof token !== 'string') {
        return sendJson(res, 400, { success: false, message: 'Missing token' })
      }
      const result = await performHtmlAction(conn, token, index)
    ...
    _wss.on('connection', (ws, req) => {
      ...
      ws.on('message', async (raw) => {
        ...
        if (msg.type === 'aiRichAction') {
          try {
            const result = await performHtmlAction(conn, msg.token, msg.index)
            ws.send(JSON.stringify({ type: 'aiRichActionResult', requestId: msg.requestId, ...result }))
          } catch (err) {
