...
export class WaVideoEngine {
    ...
    accessUnitsSent = 0;
    videoPath = null;
    // 'file' (loadVideoFile) or 'blank' (loadBlankSource) — tracks which ffmpeg
    // input the last start() used, so a bare start() after a peer-driven
    // downgrade→upgrade can resume the right one (see hasSource()/start()).
    sourceKind = null;
    constructor(config = {}) {
    ...
    /**
     * Records a solid-black lavfi source — no file needed. Used to auto-accept a
     * peer-initiated mid-call audio→video upgrade when there's no camera/loaded
     * file to send: keeps the video flow valid (peer sees a black frame, PT-97
     * keeps flowing) while audio continues uninterrupted. loadVideoFile() (if
     * called later, e.g. via a command) takes over the source on its own call to
     * start(); this only fills in when nothing else has been loaded.
     */
    ...
    start() {
        ...
        const inputArgs = this.sourceKind === 'blank'
            // -f lavfi color source: no file/camera required, just a constant black
            // frame at our target size/fps. '-re' still paces it to real-time so
            // access units arrive here at the right cadence, same as a file source.
    ...
    /**
     * Kills the running ffmpeg process. By default also forgets the loaded
     * source (matches prior behavior — used when the call itself is ending).
     * Pass keepSource: true for a mid-call video downgrade (peer sent
     * state=6/stopped): this stops sending without discarding a loaded file,
     * so a later re-upgrade (state=11) resumes the same video via start()
     * rather than falling back to the blank source or requiring a reload.
     */
