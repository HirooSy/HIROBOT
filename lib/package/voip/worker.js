...
import { VoipClient } from './index.js';
import '../../config.js'
const AUTH_DIR = global.settings.connection.caller.file;

// Log level for the voip package's internal logger: 'trace'/'debug' include
// raw wire-protocol chatter (every SCTP packet, DTLS record, etc — very
// noisy); 'media' shows call/media status only (accepted, sent/dropped
// counts, relay selection, call stats) with the wire noise suppressed;
// 'warn'/'error' show only problems.
const VOIP_LOG_LEVEL = 'warn';
...
async function runPairOnly({ pairingNumber, customPairingCode }) {
    ...
    try {
        ...
        client = new VoipClient({ authDir: AUTH_DIR, pairingCode: pairingNumber, customPairingCode, voipLogLevel: VOIP_LOG_LEVEL });
...
async function runCall({ phoneNumber, audioSource, videoSource, durationMs, isVideo }) {
    ...
    try {
        ...
        client = new VoipClient({ authDir: AUTH_DIR, voipLogLevel: VOIP_LOG_LEVEL });
        await client.connect();
        send({ type: 'connected' });
        ...
        call.on('connected', () => {
            ...
            // Same floor+buffer fix as simple.js's scheduleSafetyTimeout, and for
            // the same reason: this timer's own process.exit(1) is a second,
            // independent way the same too-short durationMs could end a call
            // early — fixing only the parent's copy of this formula would have
            // left this one still able to cut calls off at the old, wrong time.
            global.__voipRescheduleSafetyTimer?.(Math.max(durationMs ?? 120_000, 45_000) + 60_000);
        });
