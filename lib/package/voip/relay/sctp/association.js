...
export class SctpAssociation extends EventEmitter {
  ...
  #audioQueue = [];
  #videoQueue = [];
  // Real inbound TSN tracking for SACK, replacing the previous
  // "cumulativeTsnAck: tsn" (the just-arrived packet's own TSN, re-sent
  // as if it were the running cumulative point). That meant every SACK
  // reported whatever TSN happened to arrive most recently - with unordered
  // delivery (see #pumpSendQueue) packets routinely arrive out of sequence,
  // so a SACK could go 105, then 103, then 108: the peer sees the
  // "cumulative" point move backwards and forwards non-monotonically,
  // which RFC 4960 never allows and gives it no correct way to interpret.
  // #inboundCumulativeTsn is the actual highest TSN such that it and every
  // TSN before it have been received with no gap; #inboundReceivedAheadTsns
  // holds TSNs received past a gap (e.g. peer's own audio-priority sending
  // means TSNs routinely complete out of order), reported as RFC 4960 Gap
  // Ack Blocks so the sender can stop retransmitting them instead of
  // resending already-delivered data forever - see #buildAndSendSack.
  ...
  #handleChunk(chunk, header) {
    switch (chunk.type) {
      case sctp.ChunkType.INIT_ACK: {
        ...
        // First TSN we'll ever receive is peerInitialTsn, so "everything
        // before it" - the cumulative point before any DATA has arrived -
        // is one less than that (wrapping is fine here: TSNs are compared
        // with the wraparound-safe helpers everywhere this value is used).
  ...
  // Advances #inboundCumulativeTsn past any run of consecutive TSNs that's
  // now complete (including ones that arrived earlier as "ahead" gaps and
  // are now bridged by this TSN), and tracks any TSN received out of order
  // as a gap so it can be reported via Gap Ack Blocks instead of silently
  // waiting for #inboundCumulativeTsn to reach it on its own.
  #recordInboundTsn(tsn) {
    if (sackAcksTsn(this.#inboundCumulativeTsn, tsn)) {
      // Already covered by the cumulative point (a genuine duplicate, or a
      // TSN we'd already recorded as an ahead-gap and are now re-seeing) -
      // nothing to update.
      return;
    }
  ...
  // Builds a SACK from the real inbound TSN state - the cumulative point
  // (#inboundCumulativeTsn) plus Gap Ack Blocks for any TSNs received past
  // it out of order (RFC 4960 Sec 3.3.4). Without the gap blocks, a TSN
  // received ahead of a gap would never be acknowledged until the gap
  // itself filled in, so the sender would keep retransmitting data the
  // peer already has - wasted bandwidth, and worse, the sender's own
  // #inFlight entry for it never clears, so that slot in the shared
  // audio/video send window stays occupied indefinitely.
  #buildAndSendSack() {
    const gapAckBlocks = [];
    if (this.#inboundReceivedAheadTsns.size > 0) {
      ...
      for (const tsn of sorted) {
        ...
        if (offset > 0xffff) continue; // out of representable Gap Ack Block range
  ...
  #handleSack({ cumulativeTsnAck, gapAckBlocks }) {
    ...
    // Gap Ack Blocks (RFC 4960 Sec 3.3.4) cover TSNs the peer received past
    // a gap - out-of-order delivery, which unordered chunks (see
    // #pumpSendQueue) hit routinely. Without checking these too, an #inFlight
    // entry for a TSN the peer actually already has would never clear: it'd
    // sit there getting retransmitted every RETRANSMIT_TIMEOUT_MS forever,
    // permanently occupying one of the shared SEND_WINDOW_SIZE slots (and,
    // since audio/video share that window, eventually starving both) even
    // though the data was successfully delivered on the first try.
