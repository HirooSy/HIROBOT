...
// NOTE: these two names are historical (guessed from a Wireshark capture
// before the actual protobuf field semantics were known) and don't
// describe what they actually are per the reference implementation:
// 0x4000 is the relay token attribute, 0x4024 is the stream descriptors
// attribute. Kept as-is (not renamed) since buildSenderSubscriptions/
// buildSSRCSubscriptionList/buildBindingRequestWithSubs below still use
// these names — renaming now risks a missed call site under time
// pressure. buildAllocateForRelay below uses them correctly regardless of
// the name.
...
// Per-slot-index (participant, layer) plan for the 9-element stream SSRC
// array from crypto/ssrc.js's deriveWasmRelayStreamSsrcs — NOT the same as
// the slot *word* each index derives from. Source of truth: meowcaller's
// stun.go wasmStreamDescriptorPlan (in turn sourced from
// oxidezap/whatsapp-rust's wacore/src/voip/stun.rs#L120-L153).
...
/**
 * Builds the WASM/Web attr 0x4024 (stream descriptors) protobuf payload for
 * the 9-element relay stream SSRC array (crypto/ssrc.js's
 * deriveWasmRelayStreamSsrcs order — index i is slot i, NOT slot word i).
 * Every all-zero SSRC slot is skipped (a 1:1 call only ever populates slot 0
 * (audio) and slot 3 (video, word 2) — the other 7 stay zero and are
 * omitted entirely, matching CreateWasmStreamDescriptors's own zero-skip).
 * This is what actually tells the relay which stream each SSRC represents;
 * WITHOUT this exact encoding the relay has no way to associate an incoming
 * SSRC with this session at all, and silently never bridges it.
 */
...
/**
 * Builds the WASM/Web DataChannel Allocate request: 0x4000 relay token +
 * 0x4024 stream descriptors (see createWasmStreamDescriptors) + 0x0016
 * relay endpoint, message-integrity, no fingerprint. This is the message
 * that actually registers our streams with the relay so it starts
 * bridging them — see the module-level comment on why this fully replaces
 * any STUN Binding Request in this codebase.
 */
...
/**
 * Encodes XOR-MAPPED-ADDRESS (RFC 5389 §15.2, IPv4 only): family(2) +
 * XOR'd port(2) + XOR'd address(4), XORed against the magic cookie bytes.
 */
...
/**
 * Builds a STUN Binding Success Response (RFC 5389 §7.3.1): echoes the
 * request's own transactionId (required — the requester matches replies to
 * requests purely by transaction ID, not by any other correlation), reports
 * mappedIp/mappedPort as XOR-MAPPED-ADDRESS, and signs with the same
 * short-term-credential key used for our own outbound requests (STUN
 * short-term credentials are symmetric per RFC 5389 §10.1.1 — the same
 * ICE password authenticates checks in both directions of a candidate
 * pair, so reusing hmacKey here is intentional, not a shortcut).
 *
 * This exists specifically to answer STUN Binding *Requests* the relay
 * sends us mid-connection (ICE consent freshness, RFC 8445 §11) — a peer
 * that stops answering these is treated as having failed connectivity,
 * independent of whatever's happening at the DTLS/SCTP layer above it.
 */
