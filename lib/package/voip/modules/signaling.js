...
export class SignalingBridge {
    ...
    #doProcessIncomingCall = async (node, voip, activeCallId) => {
        ...
        switch (usableNode.tag) {
            ...
            case "ack":
                ...
                break;
            default:
                // DIAGNOSTIC: every <call> signaling tag other than
                // offer/ack lands here and was previously forwarded to the
                // native engine with zero visibility into what it actually
                // is. Group-call participant-join/invite notifications
                // (sent by WhatsApp when someone ELSE in the call adds a
                // participant) are expected to arrive as one of these —
                // exact tag name is currently unknown, hence this log.
                // Once a real add-participant-by-someone-else is captured
                // here during testing, promote its tag to an explicit
                // `case` above (or a dedicated handler) instead of relying
                // on this log line long-term.
                if (usableNode.tag !== "terminate" && usableNode.tag !== "reject") {
                    console.log(`[ VOIP:diag ] unhandled <call> signaling tag "${usableNode.tag}" callId=${callIdForRouting} peerJid=${routedPeerJid} attrs=${JSON.stringify(usableNode.attrs)}`);
                }
    ...
    #doProcessIncomingReceipt = async (node, voip, activeCallId) => {
        ...
        if (callIdForRouting && routedPeerJid)
            this.#incomingCallPeerById.set(callIdForRouting, routedPeerJid);
        const tcToken = await this.ensureTcToken(routedPeerJid, callbackPeerJid);
        // DIAGNOSTIC: same reasoning as the <call> tag log above — this is
        // a second candidate path for participant-added-by-someone-else
        // notifications, since WhatsApp sometimes uses receipt-shaped
        // nodes for call state updates rather than the <call> stanza.
        console.log(`[ VOIP:diag ] <receipt> for call callId=${callIdForRouting} peerJid=${routedPeerJid} childTag=${receiptChild.tag} attrs=${JSON.stringify(receiptChild.attrs)}`);
