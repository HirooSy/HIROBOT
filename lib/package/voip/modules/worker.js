...
const AUTH_DIR = process.env.VOIP_AUTH_DIR || 'data/sessions/caller.session'
...
async function runCall({ phoneNumber, audioSource, videoSource, durationMs, isVideo }) {
  ...
  try {
    ...
    // DIAGNOSTIC: forwards whatever lib/voip/index.js couldn't classify
    // (see #handleCallEvent's catch-all) up to the parent process/console
    // too, so it's visible without having to watch the forked worker's
    // own stdout separately. This is how "someone else added a
    // participant to the call" is expected to be observed for now — there
    // is no confirmed/dedicated event for it yet.
    call.on('unknownCallEvent', (info) => {
      send({ type: 'unknown_call_event', eventType: info?.eventType, eventData: info?.eventData })
    })
