...
function send(msg) {
  ...
  try {
    ...
    // Parent's IPC channel closed between the process.connected check and
    // the send call (e.g. parent restarted mid-call) — nothing useful to do
    // with a message nobody will receive, so just drop it rather than
    // crashing the worker over a notification that no longer matters.
  }
}
