...
export class WasmEngine {
    ...
    constructor(config = {}) {
        ...
        const wasmPath = config.wasmPath
            ...
            : path.join(basePath, "whatsapp.wasm");
        this.#config = {
            ...config,
            wasmPath,
    ...
    // Invites/rings an additional participant into the currently active
    // call, upgrading a 1:1 call into a group call. Mirrors the
    // WAWebVoipWorkerHandler "inviteToCall" RPC (see worker-modules.js) —
    // same underlying native binding, just called directly here instead of
    // through the pthread worker's message-passing layer, matching how
    // startCall/endCall are already called directly above.
    //
    // pnUserJid / lidUserJid: the phone-number JID and LID JID of the
    // participant being invited (both are sent; the native side picks
    // whichever it needs). deviceJids: that participant's device JID list
    // (same shape as the `peerList` passed to startCall).
    inviteToCall = (pnUserJid, lidUserJid, deviceJids = []) => {
        this.#ensureInitialized();
        if (typeof this.#instance.inviteToCall !== "function") {
            throw new Error("inviteToCall is not available in this WASM build.");
        }
        const devices = this.#makeStringList(deviceJids.length ? deviceJids : [pnUserJid]);
        try {
            return this.#instance.inviteToCall(String(pnUserJid || ""), String(lidUserJid || ""), devices);
        }
        finally {
            devices?.delete?.();
        }
    };
    // Raw call/participant state as reported by the native stack. Useful
    // while wiring up group-call features: the WASM event codes for
    // participant-joined/ringing aren't documented anywhere in this
    // codebase, so during testing it's safer to poll this after an
    // inviteToCall() than to guess an eventType number in
    // #handleCallEvent. Returns whatever shape the native side gives back
    // (raw value/JSON string depending on build) — callers should log it
    // first before assuming a structure.
    getCallInfo = () => {
        this.#ensureInitialized();
    ...
    #applyDefaultAbProps = () => {
        ...
        const boolProps = {
            ...
            enable_dynamic_bitrate_cap: false,
        };
