...
export class VoipClient {
    ...
    #handleCallEvent = (eventType, eventData) => {
        if (eventType === 16 && eventData) {
            ...
            // DIAGNOSTIC: catch-all for every eventType not already mapped
            // above. Group-call participant notifications (someone else in
            // the call inviting/adding a new participant) are expected to
            // surface as one of these unmapped codes — WhatsApp's native
            // call stack reports call/participant state changes through
            // this same onCallEvent callback, just with codes this file
            // doesn't recognize yet. Logged AND re-emitted on the active
            // call so the exact code+payload can be observed during a real
            // test (someone else adding a participant to a call this bot
            // is on) without needing another code change to see it.
            let parsedEventData = eventData;
            try {
                parsedEventData = eventData ? JSON.parse(eventData) : eventData;
            }
            catch { }
            console.log(`[ VOIP:diag ] unmapped onCallEvent eventType=${eventType} eventData=${JSON.stringify(parsedEventData)}`);
            this.#activeCall?.emit("unknownCallEvent", { eventType, eventData: parsedEventData });
        }
    };
