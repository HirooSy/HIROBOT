...
export class VoipClient {
    ...
    #handleCallEvent = (eventType, eventData) => {
        if (eventType === 16 && eventData) {
            ...
            // Catch-all for every eventType not already mapped above (audio
            // level updates, banner/UI state, group-call participant
            // notifications, etc). Re-emitted on the active call rather than
            // logged, so nothing spams the console/chat by default, but the
            // raw eventType+payload is still available to any listener that
            // wants it (see plugins/owner/call.js for how this was
            // previously wired up for debugging).
