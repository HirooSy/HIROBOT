...
export class WaCallManager extends EventEmitter {
    ...
    createSession(info, options = {}) {
        ...
        const session = new WaCallMediaSession({
            ...
            delegate: {
                ...
                // videoState: { active, upgrade, raw } — active/upgrade mirror
                // meowcaller's VideoState struct; upgrade marks a peer-initiated
                // mid-call transition we just auto-accepted (state 11/3) so
                // listeners can tell "video turned on because they asked" apart
                // from a plain state=1 confirmation.
                emitVideoState: (call, videoState) => this.emit('call_video_state', call, videoState),
                // au: one reassembled Annex-B access unit of the peer's video —
                // pixels are the caller's problem (feed to a decoder/display);
                // this bot never decodes video itself, same as it never encodes it.
    ...
    resolveSessionForOfferAck(node) {
        ...
        if (outgoing.length === 1)
            return outgoing[0];
        // trace, not debug — this fires for every late/duplicate offer-ack
        // relay-update packet once a call has already progressed past
        // Initiating/Ringing (expected, harmless), and was burying the
        // one-time connection-lifecycle lines (ice binding phase done, dtls
        // handshake complete) under repeats of this instead.
