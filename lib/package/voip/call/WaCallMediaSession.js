...
export class WaCallMediaSession {
    ...
    startMediaFlow() {
        ...
        setImmediate(() => {
            // See the -ss comment in WaVideoEngine.start(): video's own
            // startup latency (this setImmediate deferral, plus spawning
            // its own ffmpeg process) otherwise leaves it permanently
            // behind audio, which already started reading from position 0
            // the instant startCapture() above returned. Passing how much
            // time has actually elapsed since then lets the video engine
            // seek its input to compensate.
            const elapsedMs = Date.now() - audioStartedAtMs;
            this.videoEngine.start(elapsedMs);
        });
    }
}
