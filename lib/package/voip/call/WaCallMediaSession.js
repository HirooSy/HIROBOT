...
export class WaCallMediaSession {
    ...
    // Mid-call video transition state (standalone <call><video state=N/></call>
    // stanzas, distinct from the from-start <video> child on offer/accept — see
    // signaling.js VideoState). peerVideoUpgradePending mirrors meowcaller's
    // engine.peerVideoUpgrade: true while a peer-initiated upgrade (state 11) has
    // been auto-accepted by us but we're still waiting to actually see PT-97
    // packets, purely for diagnostics/observability.
    ...
    /**
     * Handles an inbound standalone <call><video state=N/></call> stanza — the
     * peer's mid-call video on/off, distinct from the from-start <video> child
     * on offer/accept. Auto-accepts an upgrade with no user-facing command:
     * whatever we have loaded (or, absent that, a black-screen fallback) is
     * brought up so audio keeps flowing and the video flow itself stays valid,
     * mirroring what tapping the camera icon in the real WhatsApp client does
     * on the other end. bridge.js has already sent the required
     * <ack class="call" type="video"> before this runs (routeCallStanza uses
     * the stanza's own tag for the ack type) — matching the fix meowcaller
     * needed for this same transition (a generic typeless ack makes the sender
     * revert the upgrade after ~5s).
     */
    async handleCallVideo(node) {
        ...
        switch (state) {
            ...
            case VideoState.UpgradeCancel: {
                // Peer withdrew/rejected before we finished bringing our side up —
                // make sure we don't leave a half-started video pipeline behind.
                this.disableVideoMidCall({ keepSource: true });
                this.delegate.emitVideoState?.(this.info, { active: false, upgrade: false, raw: state });
                break;
            }
    ...
    /**
     * Brings up the video send/receive pipeline on an already-active audio
     * call, without restarting the call or re-touching audio. No-ops if video
     * is already up. Mirrors what initMedia() does for a from-start video
     * call, plus initSrtpKeys()'s video branch, but callable mid-call.
     */
    /**
     * Caller-driven counterpart to handleCallVideo()'s peer-driven upgrade:
     * WE initiate a mid-call audio→video upgrade (e.g. a playlist item
     * switching from audio to video) rather than reacting to the peer's own
     * <video state=11/> arriving. Sends the same UpgradeRequestV2 stanza a
     * real WhatsApp client sends when the camera icon is tapped mid-call,
     * then brings up the local pipeline immediately rather than waiting for
     * the peer's UpgradeAccept — matches how enableVideoMidCall() already
     * doesn't block on any round trip before starting to encode.
     */
    ...
    async enableVideoMidCall() {
        ...
        this.videoRtpStream = new VideoRtpStream(this.videoSsrc, videoRtpDurationSamples(this.videoEngine.frameDurationMs));
        // Same per-JID SRTP key material as audio (see initSrtpKeys) — audio's
        // session is already live at this point since this is a mid-call
        // upgrade, so we just derive video's independent SrtpContext from the
        // same keying, not from any freshly-negotiated key.
        if (this.srtpSession) {
        ...
        this.videoAssembler = new H264AccessUnitAssembler();
        // If nothing is loaded (no camera, no loadVideo() call), fall back to a
        // black screen so the flow stays valid and audio is uninterrupted — the
        // requested behavior for an auto-accepted upgrade with no video source.
    ...
    /**
     * Caller-driven counterpart to the peer-driven state=6 path in
     * handleCallVideo(): WE stop sending video (e.g. a playlist item
     * switching from video to audio) rather than reacting to the peer
     * turning theirs off. Sends the Stopped stanza a real client sends when
     * the camera icon is tapped off mid-call, then tears down the local
     * pipeline. keepSource is forwarded so a playlist that goes
     * video→audio→video again resumes the same file (see
     * disableVideoMidCall's own doc comment).
     */
    ...
    /**
     * Tears down the video send/receive pipeline while leaving audio and the
     * call itself untouched — a mid-call downgrade back to audio-only.
     * keepSource: true (used for a peer-driven state=6) leaves a previously
     * loaded file in place so a later re-upgrade resumes it instead of
     * restarting or falling back to blank; full cleanup() (call end) still
     * discards everything via videoEngine.stop() with no options.
     */
    ...
    /**
     * Demuxes and decodes one inbound H.264 (PT 97) RTP packet from the peer:
     * SRTP-unprotect → depacketize/reassemble into an Annex-B access unit on
     * the marker bit → forward to whatever's listening (see
     * WaCallManager.handleCallVideo / the public Call-level video sink).
     * Silently drops if video isn't up yet on our side (videoSrtpSession null)
     * — e.g. a packet that raced ahead of our own upgrade handling.
     */
    ...
    startMediaFlow() {
        ...
        //
        // Also deferred by one event-loop turn (setImmediate) past audio's
        // startCapture() above, not called in the same tick: child_process.spawn()
        // does a synchronous fork+exec that can take tens of ms, and audio's very
        // first setInterval tick lands moments after this function returns. Doing
        // both in the same tick meant that occasional spawn latency delayed
        // audio's first captures by 1-1.3s (see the "captured audio frame gap
        // larger than expected" warning at call start) — audio settles onto its
        // own cadence fine once running, so pushing the spawn one turn later gives
        // it a clear head start instead of contending with it.
