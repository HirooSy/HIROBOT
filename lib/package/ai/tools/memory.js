import { ctx, ensureBrainGroupSlot, getPinnedNotesReadOnly, loadBrain, saveBrain } from '../mcp.js';
export default [
{
    name: 'remember',
...
{
    name: 'list_learned',
    description: `Tampilkan semua yang sudah dipelajari ${global.settings.botname}. Bisa filter per kategori.`,
    parameters: {
    ...
    execute: async ({ category } = {}) => {
        ...
        const items = category
            ...
            : brain.learned
        if (!items.length) return `${global.settings.botname} belum punya memori${category ? ` kategori "${category}"` : ''}.`
        const grouped = {}
        items.forEach(m => {
            if (!grouped[m.category]) grouped[m.category] = []
            grouped[m.category].push(m.key)
        })
        let out = `*${global.settings.botname} Brain* (${items.length} memori)\n\n`
...
{
    name: 'forget',
    description: `Hapus memori tertentu dari brain ${global.settings.botname}.`,
    parameters: {
...
{
    name: 'log_failure',
    description: `Catat percobaan yang gagal ke brain agar tidak diulangi dengan cara yang sama. ${global.settings.botname} belajar dari kesalahan.`,
    parameters: {
