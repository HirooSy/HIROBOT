...
const _mistralSessions = new Map();
async function askMistralChat(senderJid, text) {
    let session = _mistralSessions.get(senderJid);
    if (!session) {
        const firstMessage = buildMistralIdentityPrompt() + '\n\n[PESAN PERTAMA USER]\n' + text;
        const auth = await mistralGetSession();
        const chatId = await mistralCreateChat(firstMessage, auth);
        const reply = await mistralStream(firstMessage, auth, chatId, true);
        _mistralSessions.set(senderJid, { auth, chatId });
        return reply;
    }
    try {
        return await mistralStream(text, session.auth, session.chatId, false);
    } catch (err) {

        _mistralSessions.delete(senderJid);
        const firstMessage = buildMistralIdentityPrompt() + '\n\n[PESAN PERTAMA USER]\n' + text;
        const auth = await mistralGetSession();
        const chatId = await mistralCreateChat(firstMessage, auth);
        const reply = await mistralStream(firstMessage, auth, chatId, true);
...
export function resetMistralSession(jid) {
    _mistralSessions.delete(jid);
}
...
export async function handleError(conn, m, err, pluginName = 'unknown') {
    ...
    const autoHealDisabled = global.settings?.ai?.autoheal !== true || process.env.DISABLE_AUTO_HEAL === 'true';
    if (autoHealDisabled) {
        try {
            await conn.sendMessage(chat, {
                text: isOwner
...
async function runMistralFallback(senderJid, text) {
    ...
    if (trimmed.toLowerCase() === 'reset') {
        resetMistralSession(senderJid);
        return { type: 'text', text: '🔄 Sesi chat direset.' };
    }
    try {
        const reply = await askMistralChat(senderJid, trimmed);
        if (!reply) {
...
export async function runAgent(conn, m, text, opts = {}) {
    ...
    if (!apiKey && !getApiKeys().length) {
        return runMistralFallback(senderJid, text);
    }
