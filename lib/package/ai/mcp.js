...
async function _grStreamOnce(prompt, auth, chatMetadata, onChunk, imageBuffers = []) {
    auth.reqId = (auth.reqId || 10000) + 100000;
    const metadata = chatMetadata ? [...chatMetadata] : ['', '', '', null, null, null, null, null, null, ''];

    // Build prompt part — support image
    let promptPart;
    if (imageBuffers && imageBuffers.length > 0) {
        // Image dikirim inline sebagai base64 dalam payload
        const imageParts = imageBuffers.map(({ buffer, mimeType }) => ({
            imageData: { data: buffer.toString('base64'), mimeType },
        }));
        promptPart = [prompt, 0, null, imageParts, null, null, 0];
    } else {
        promptPart = [prompt, 0, null, null, null, null, 0];
    }
    ...
    const streamRes = await _grHttpRequest(
        ...
        { method: 'POST', headers: {
            ...
            cookie: _buildCookieStr(auth.cookies),
            ..._GR_MODEL_HEADERS.BASIC_FLASH,
        }},
        body.toString()
    );
    _syncCookies(auth.cookies, streamRes.headers['set-cookie']);

    return new Promise((resolve, reject) => {
        let accText = '', lastSentText = '', buf = '', updatedMeta = metadata;
        const tid = setTimeout(() => { streamRes.destroy(); reject(new Error('Response timeout')); }, 60000);
        ...
        streamRes.on('data', chunk => {
            try {
                ...
                while (true) {
                    ...
                    if (nl === -1) break;
                    const sizeStr = buf.substring(0, nl).trim();
                    const size = parseInt(sizeStr, 10);
                    if (isNaN(size)) { buf = buf.substring(nl + 1); continue; }
                    if (buf.length < nl + size) break;
                    const frame = buf.substring(nl, nl + size);
                    buf = buf.substring(nl + size);
                    let fd; try { fd = JSON.parse(frame); } catch { continue; }
                    const envelope = Array.isArray(fd) ? fd : [fd];
                    for (const item of envelope) {
                        const innerStr = item?.[2]; if (!innerStr) continue;
                        let pj; try { pj = JSON.parse(innerStr); } catch { continue; }
                        // Check error code
                        const ec = pj?.[3]?.[0]?.[0]?.[0]?.[0];
                        if (ec) {
                            clearTimeout(tid);
                            reject(new Error(`GeminiGuest API error: ${ec}`));
                            return;
                        }
        ...
        streamRes.on('end', () => {
            ...
            resolve({ text: accText, metadata: updatedMeta });
        });
        streamRes.on('error', err => { clearTimeout(tid); reject(err); });
    });
}
// ─────────────────────────────────────────────────────────────────────────────
...
Untuk panggil tool, balas HANYA dengan JSON ini (tidak ada teks lain):
{"__tool_call":{"name":"nama_tool","args":{"param1":"val1"}}}
Kalau mau panggil tool: SELURUH balasanmu harus JSON di atas saja.
Setelah terima hasil tool, boleh panggil tool lagi atau jawab user dengan teks biasa.
...
async function mcpLoopGemRev(jid, history, onStep, stepLang = 'id') {
    ...
    if (!entry.initialized) {
        ...
        const toolSchema = buildGemRevToolSchema();
        firstPrompt = (toolSchema ? toolSchema + '\n\n' : '') + userText;
    }
    ...
    for (let i = 0; i < MAX_LOOPS; i++) {
        let responseText;
        try {
            // Pakai _grStreamOnce — metadata fresh tiap turn (tidak ada cid/rid lama → bebas 1096)
            // chatMeta null → DEFAULT_METADATA, Gemini treat sebagai percakapan baru tiap turn
            const result = await _grStreamOnce(currentPrompt, entry.auth, null, null, currentImages);
            responseText = result?.text || '';
            // Setelah turn pertama, jangan kirim image lagi
            currentImages = [];
        } catch (err) {
            // Session rusak — hapus dan rethrow
            _deleteGemRevSession(jid);
            throw err;
        }

        const toolCall = parseGemRevToolCall(responseText);
        if (!toolCall) {
        ...
        currentPrompt = `[HASIL TOOL: ${fc.name}]\n${finalOutput}\n\nLanjutkan — kalau perlu panggil tool lagi, balas JSON; kalau sudah selesai, jawab user dengan teks biasa.`;
