...
1.5. DILARANG KERAS mengarang alasan gagal (mis. "lagi kena limit", "API-nya
   ...
   panggil tool yang sesuai (view_link_post, download_tiktok, view_website,
...
6. run_plugin BUKAN shell_exec — itu cuma manggil fitur bot yang sudah jadi
   ...
   run_plugin sendiri.
   PENTING: yang BENAR-BENAR diblok hanya command yang DIAWALI prefix
   raw-code: "$" (exec/shell), "<<" atau "<" (eval JS). Contoh yang diblok:
   run_plugin("$ ls"), run_plugin("<< 1+1"), run_plugin("< code").
...
10. Kalau permintaan user AMBIGU/TERBUKA dan butuh klarifikasi (mis. "carikan
    ...
    PANGGIL TOOL send_buttons — bukan cuma menuliskan daftar pilihan sebagai
    bullet list teks biasa.
...
- download_tiktok, download_instagram, download_youtube, play_youtube, download_facebook, download_twitter
...
LINK SOSMED — KAPAN PAKAI APA:
  ...
  → Ada kata "download", "unduh", "kirim", "minta video/foto/audio" → pakai download_tiktok / download_instagram / download_youtube / dll LANGSUNG, tanpa nanya dulu.
...
registerTool({
    ...
    execute: async ({ file_path, offset = 0 }) => {
        ...
        if (!fs.existsSync(abs)) {
            ...
            if (matches.length > 1 && _conn && _currentJid) {

                try {
                    await callTool('send_buttons', {
                        body: `File "${file_path}" nggak ketemu persis, tapi nemu ${matches.length} file dengan nama yang sama. Yang mana nih?`,
                        buttons: matches.slice(0, 8).map(m => ({ type: 'reply', label: m.length > 24 ? '…' + m.slice(-23) : m, value: `baca file ${m}` }))
                    })
                    return `File "${file_path}" tidak ketemu persis. Ditemukan ${matches.length} kandidat dengan nama sama, sudah ditawarkan ke user lewat tombol pilihan — tunggu user pilih, jangan langsung nebak salah satu.`
                } catch (_) {
                    return `File tidak ditemukan: ${file_path}\nKandidat dengan nama sama: ${matches.join(', ')}`
                }
            }
...
registerTool({
    name: 'whois',
    description: 'Cari tahu identitas seseorang dari JID/nomor/mention -- pakai ini kalau user tanya "siapa ini @udin" atau minta info soal member tertentu. Mengembalikan nama (kalau dikenali database), status admin di grup aktif (kalau chat ini grup), status owner bot, dan status registrasi. Terima nomor telepon, JID lengkap (@s.whatsapp.net/@lid), atau kalau ini merespons pesan yang me-mention seseorang, JID mention-nya bisa dilihat dari konteks pesan yang diberikan ke kamu.',
    parameters: {
        jid: { type: 'string', description: 'Nomor telepon atau JID orang yang mau dicari tahu (contoh: "628123456789" atau "628123456789@s.whatsapp.net" atau "...@lid").', required: true }
    },
    execute: async ({ jid }) => {
        if (!_conn) return 'WA connection not ready'
        if (!jid) return 'jid wajib diisi.'
        const target = jid.includes('@') ? jid : jid.replace(/\D/g, '') + '@s.whatsapp.net'

        let identity
        try {
            identity = await getUserIdentity(target, db, _conn)
        } catch (e) {
            return `Gagal cari identitas: ${e.message}`
        }

        const lines = [
            `Nomor: ${identity.number}`,
            `Nama: ${identity.name}`,
            `Owner bot: ${identity.isOwner ? 'ya' : 'tidak'}`,
            `Terdaftar di bot: ${identity.registered ? 'ya' : 'belum pernah interaksi'}`
        ]

        if (_currentJid?.endsWith('@g.us')) {
            try {
                const meta = await _conn.groupMetadata(_currentJid)
                const participant = meta.participants?.find(p => matchParticipant(_conn, p, target))
                if (participant) {
                    lines.push(`Status di grup ini: ${participant.admin === 'superadmin' ? 'owner grup' : participant.admin === 'admin' ? 'admin' : 'member biasa'}`)
                } else {
                    lines.push('Status di grup ini: bukan/tidak ditemukan sebagai member grup ini.')
                }
            } catch (e) {
                lines.push(`(gagal cek status di grup ini: ${e.message})`)
            }
        }
...
//
//   'blocked' → sistem/berbahaya (eval/exec/shell, session/pairing, migrasi
...
//
// Plugin file BOLEH override manual dengan `handler.risk = 'low'|'medium'|'high'|'blocked'`
// kalau deteksi pattern di bawah kurang tepat untuk plugin tsb — nilai manual
// ini SELALU menang duluan sebelum pattern-matching apapun.
const RISK_BLOCKED_PATTERNS = [
    /\b(eval|exec|shell|terminal|cmd)\b/i,
...
function classifyPluginRisk(name, plugin) {
    ...
    if (typeof plugin.risk === 'string' && ['low', 'medium', 'high', 'blocked'].includes(plugin.risk)) {
        return { level: plugin.risk, reason: `Level risiko di-set manual oleh plugin (handler.risk = "${plugin.risk}").` }
    }

    const identity = pluginIdentity(name, plugin)

    if (plugin.rowner === true || RISK_BLOCKED_PATTERNS.some(p => p.test(identity))) {
        return { level: 'blocked', reason: 'Termasuk kategori sistem/sensitif (eval/exec/session/db/secret) atau rowner-only — tidak pernah dijalankan otomatis oleh AI.' }
    }
...
async function execPluginCommand(command, argsStr = '', { confirmed = false } = {}) {
    ...
    if (/^(\$|<<?)/.test(command.trim())) {
        throw new Error(`Command "${command}" is a raw-code prefix (eval/exec) and cannot be run automatically.`)
    }
    ...
    const rawCodeRe = /(^|[\\/])(eval|exec)\.js$/i
...
Setiap command otomatis dikategorikan ke 1 dari 4 level risiko (cek dulu pakai check_plugin_risk kalau ragu, atau lihat badge-nya di list_plugins):
  ⛔ blocked → sistem/berbahaya (eval/exec/shell, session, secret/token, migrasi db, dst). Tool ini akan MENOLAK sendiri, jangan dipaksa.
...
registerTool({
    ...
    description: 'Download video atau foto TikTok tanpa watermark lewat plugin .tiktok bot dan langsung kirim ke user. Gunakan saat ada URL tiktok.com atau vt.tiktok.com',
    parameters: {
        url: { type: 'string', description: 'URL video TikTok', required: true }
    },
    execute: async ({ url }) => {
        try {
            await execPluginCommand('tiktok', url)
            return 'TikTok diproses lewat plugin .tiktok, hasil dikirim langsung ke chat ini.'
        } catch (e) {
            return `Gagal download TikTok: ${e.message}`
        }
    }
})
...
registerTool({
    ...
    description: 'Download video atau foto Instagram Reels/Post lewat plugin .ig bot dan langsung kirim ke user. Gunakan saat ada URL instagram.com',
    parameters: {
        url: { type: 'string', description: 'URL Instagram', required: true }
    },
    execute: async ({ url }) => {
        try {
            await execPluginCommand('ig', url)
            return 'Instagram diproses lewat plugin .ig, hasil dikirim langsung ke chat ini.'
        } catch (e) {
            return `Gagal download Instagram: ${e.message}`
        }
    }
})
...
registerTool({
    ...
    description: 'Download video YouTube/Shorts (MP4) lewat plugin .ytv bot dan langsung kirim ke user. Gunakan saat ada URL youtube.com/youtu.be dan user mau video.',
    parameters: {
        url: { type: 'string', description: 'URL YouTube', required: true }
    },
    execute: async ({ url }) => {
        try {
            await execPluginCommand('ytv', url)
            return 'YouTube video diproses lewat plugin .ytv, hasil dikirim langsung ke chat ini.'
        } catch (e) {
            return `Gagal download YouTube: ${e.message}`
        }
    }
})
...
registerTool({
    name: 'play_youtube',
    description: 'Cari dan kirim audio/lagu dari YouTube lewat plugin .play bot sebagai audio message. Gunakan saat user minta putar lagu, carikan lagu, atau download MP3. Cukup kasih judul lagu, tidak perlu URL.',
    parameters: {
        query: { type: 'string', description: 'Judul lagu atau URL YouTube', required: true }
    },
    execute: async ({ query }) => {
        try {
            await execPluginCommand('play', query)
            return `Lagu "${query}" diproses lewat plugin .play, hasil dikirim langsung ke chat ini.`
        } catch (e) {
            return `Gagal cari/putar lagu: ${e.message}`
        }
    }
})
...
registerTool({
    ...
    description: 'Download video Twitter/X lewat plugin .twitter bot dan langsung kirim ke user. Gunakan saat ada URL twitter.com atau x.com',
    parameters: {
        url: { type: 'string', description: 'URL Twitter/X', required: true }
    },
    execute: async ({ url }) => {
        try {
            await execPluginCommand('twitter', url)
            return 'Twitter/X diproses lewat plugin .twitter, hasil dikirim langsung ke chat ini.'
        } catch (e) {
