...
function buildRoutes(conn) {
  ...
  get('/api/botInfo', async (req, res) => {
    sendJson(res, 200, { name: process.env.BOT_NAME, icon: global.settings.icon })
  })
  ...
  post('/api/buy-premium', async (req, res, ctx) => {
    try {
      ...
      sendJson(res, 200, { success: true, message: `Premium extended by ${weeks} week(s)!`, gems: user.gems, premium: user.premium, premiumTime: user.premiumTime })
    } catch (err) {
