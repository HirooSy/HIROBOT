...
class MongoDB {
  ...
  async _writeUnlocked(data) {
    ...
    // Each collection's bulkWrite+deleteMany is independent of the others,
    // so running them in parallel instead of one-by-one cuts total write
    // time roughly from O(numCollections) round-trips down to the slowest
    // single collection. This matters most during shutdown, where
    // flushDatabaseAndExit only allows a few seconds before force-exiting -
    // a sequential loop over many collections could easily blow past that
    // budget even though no individual write was slow.
...
// --- Debounced autosave ---------------------------------------------------
// Per-message mutations (exp, limit, chat counters, etc.) happen directly on
// database.data in-memory and are only durable once .write() runs. Calling
// write() on every single message would hammer disk I/O on a busy bot, so
// this coalesces bursts of activity into a single write after a short quiet
// period — while still guaranteeing a write happens soon after real changes,
// instead of relying solely on the 20-minute interval in lib/main.js.
const DB_AUTOSAVE_DEBOUNCE_MS = 15_000  // flush 15s after the last change
const DB_AUTOSAVE_MAX_WAIT_MS = 60_000  // but never wait longer than this
