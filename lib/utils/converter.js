...
async function decodeImageObject(img) {
  ...
  if (base.kind === 'jpeg') {
    const { data, info } = await jimp.toRaw(base.buffer, { channels: 3 })
    baseBuf = data
  ...
  if (img.smask) {
    ...
    let alphaBuf
    if (mask.kind === 'jpeg') {
      alphaBuf = (await jimp.toRaw(mask.buffer, { channels: 1 })).data
    } else if (mask.channels === 1) {
      alphaBuf = mask.buffer
    } else {
      // mask N-channel (2/3/4) -> luminance 1 channel; langsung dari piksel, tanpa encode ulang
      const m = jimp.fromRaw({ data: mask.buffer, width: img.smask.width, height: img.smask.height, channels: mask.channels })
      alphaBuf = m.greyscaleRaw()
    }
  ...
  return jimp.fromRaw({ data: baseBuf, width: img.width, height: img.height, channels: baseChannels })
    .getBuffer('image/jpeg', { quality: 92 })
}
...
async function img2pdf(input) {
  ...
  for (const raw of items) {
    ...
    pages.push({ jpeg, width: img.width, height: img.height })
  }
