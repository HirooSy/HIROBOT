...
function buildWebpFromChunks(chunks) {
    ...
    for (const { fourCC, data } of chunks) {
        ...
        if (size % 2 !== 0) parts.push(Buffer.from([0])) 
    }
...
const { fileTypeFromBuffer, fileTypeStream } = Helper
...
// Simple builder for an a2ui "bloksWidget" card: title/subtitle image, an
// optional row of gallery thumbnails, a few info lines, and a couple of
// link buttons. Callers just pass plain data — no need to hand-write the
// a2ui component graph.
//   opts.widget = {
//     title, subtitle,               // hero card text
//     image,                          // hero image URL (string)
//     gallery: [url, url, ...],       // optional row of small images
//     info: [line1, line2, ...],      // optional info section (each a string)
//     buttons: [{ text, url }, ...]   // optional row of link buttons
//   }
function nfBuildBloksWidget(widget = {}) {
    const {
        title = '', subtitle = '',
        image = '',
        gallery = [],
    ...
    if (image || title || subtitle) {
        components.push(
            { id: 'card_hero', component: 'Card', child: 'hero_content' },
            { id: 'hero_content', component: 'Column', justify: 'center', align: 'center', children: ['hero_image', 'hero_title', 'hero_subtitle'].filter((_, i) => [image, title, subtitle][i]) },
        )
        if (image) components.push({ id: 'hero_image', component: 'Image', url: image, variant: 'header', fit: 'cover' })
        if (title) components.push({ id: 'hero_title', component: 'Text', text: String(title), variant: 'h1' })
        if (subtitle) components.push({ id: 'hero_subtitle', component: 'Text', text: String(subtitle), variant: 'caption' })
        children.push('card_hero')
    }

    if (gallery.length) {
        children.push('divider_gallery', 'row_gallery')
        components.push({ id: 'divider_gallery', component: 'Divider', axis: 'horizontal' })
        components.push({
            id: 'row_gallery', component: 'Row', justify: 'center', align: 'center',
            children: gallery.map((_, i) => `gallery_${i}`)
        })
        gallery.forEach((url, i) => {
            components.push({ id: `gallery_${i}`, component: 'Image', url, variant: 'avatar', fit: 'cover' })
        })
    }

    if (info.length) {
        children.push('divider_info', 'row_info')
        components.push({ id: 'divider_info', component: 'Divider', axis: 'horizontal' })
        components.push({ id: 'row_info', component: 'Column', justify: 'center', align: 'center', children: info.map((_, i) => `info_${i}`) })
        info.forEach((line, i) => {
            components.push({ id: `info_${i}`, component: 'Text', text: String(line), variant: i === 0 ? 'body' : 'caption' })
        })
    }

    if (buttons.length) {
        children.push('divider_btn', 'row_btn')
        components.push({ id: 'divider_btn', component: 'Divider', axis: 'horizontal' })
        components.push({ id: 'row_btn', component: 'Row', justify: 'center', align: 'center', children: buttons.map((_, i) => `button_${i}`) })
        buttons.forEach((btn, i) => {
            components.push(
                { id: `button_${i}`, component: 'Button', child: `button_text_${i}`, variant: i === 0 ? 'primary' : 'borderless', action: { call: 'openUrl', args: { url: String(btn.url || '') } } },
                { id: `button_text_${i}`, component: 'Text', text: String(btn.text || 'Open'), variant: 'body' }
            )
        })
    }

    components.unshift({ id: 'root', component: 'Column', justify: 'center', align: 'center', children })
...
function isNativeFlowStyle(opts = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendProduct: {
            async value(jid, media, title = '', description = '', quoted, options = {}) {
                const {
                    businessOwnerJid = this.user?.jid || conn.user?.jid,
                ...
                if (!items.length) throw new TypeError('conn.sendProduct requires at least one media item')

                const productImageCount = items.length
                const results = []

                for (const item of items) {
                    const file = await conn.getFile(item)
                    let buffer
                    try {
                        ...
                        const content = {
                            productMessage: {
                                product: {
                                    ...
                                    ...(priceAmount1000 != null ? { priceAmount1000: String(priceAmount1000) } : {}),
                                    ...(retailerId ? { retailerId } : {}),
                                    ...(url ? { url } : {}),
                                    ...(salePriceAmount1000 != null ? { salePriceAmount1000: String(salePriceAmount1000) } : {}),
                                },
                                businessOwnerJid,
                                ...rest,
                            }
                        ...
                        results.push(await conn.relayMessage(jid, content, quoted ? { quoted } : {}))
                    } finally {
                        file.clear()
                    }
                }
        ...
        try {
          ...
          var ab = await jimp.resize(input, width, height, { out: 'png' })
          resolve(ab)
        } catch (e) { reject(e) }
       })
      }
     ...
     try {
       var input = await toImageBuffer(buffer)
       var a = await jimp.extract(input, { left: ukur1, top: ukur2, width: ukur3, height: ukur4 }, 'jpeg')
       resolve(a)
     } catch (e) { reject(e) }
  })
  }},
...
`.trim()
       ...
       sendFooter: {
        ...
        async value(jid, image, text, title, description, preview = 0, quoted, options = {}) {
            ...
            // baileys-lite cuma bisa decode JPEG/PNG lewat codec bawaannya; format lain (WebP dkk,
            // termasuk foto profil WA) dia coba cari lewat `import('jimp')` dari npm - yang tidak
            // pernah terpasang di sini. Precompute thumbnail-nya sendiri pakai jimp NATIVE dari
            // canvas.js (bukan paket npm) supaya prepareWAMessageMedia tidak perlu (dan tidak gagal)
            // mencoba decode sendiri; jpegThumbnail yang sudah ada di uploadData langsung dipakai.
            let jpegThumbnail
            ...
            try {
                ...
                const thumb = img.clone().resize(32, Math.round(32 * img.height / img.width))
                jpegThumbnail = await thumb.getBuffer('image/jpeg', { quality: 50 })
            } catch (e) {
                conn.logger?.warn?.('sendUrlPreview: gagal generate thumbnail via jimp native: ' + (e?.message || e))
            }

            const { imageMessage } = await prepareWAMessageMedia({
                image: imageSource,
                ...(jpegThumbnail && { jpegThumbnail }),
                ...(thumbWidth && thumbHeight && { width: thumbWidth, height: thumbHeight })
            }, {
                upload: conn.waUploadToServer,
        ...
        if (isNativeFlowStyle(opts)) {
            ...
            const content = {
                interactiveMessage: {
                    ...
                    ...(opts.widget ? { bloksWidget: nfBuildBloksWidget(opts.widget) } : {})
                }
            }
        ...
        if (isDocument) {
            ...
            if (!mimetype) {
                try {
                    const meta = jimp.metadata(buffer)
...
async function resolveLidViaUSync(lidNumber, conn) {
    if (typeof conn?.query !== 'function') {
        console.warn(`[LID] conn.query tidak tersedia — skip USync lookup (${lidNumber})`)
        return null
    }
    if (!USyncQuery || !USyncUser || !getBinaryNodeChild) {
        console.warn(`[LID] USync helpers tidak ke-export dari versi baileys ini — skip step 2 (${lidNumber})`)
        return null
    }
    try {
        const usyncQuery = new USyncQuery()
            ...
            .withMode('query')
            .withLIDProtocol()
            .withUser(new USyncUser().withLid(lidNumber))

        const userNodes = usyncQuery.users
            .map(user => ({
                tag: 'user',
                attrs: { jid: user.lid },
                content: usyncQuery.protocols.map(p => p.getUserElement(user)).filter(Boolean)
            }))
            .filter(node => node.content.length > 0)

        if (userNodes.length === 0) return null

        const tag = typeof conn.generateMessageTag === 'function'
        ...
        const iq = {
            tag: 'iq',
            attrs: {
                to: S_WHATSAPP_NET || '@s.whatsapp.net',
                type: 'get',
                xmlns: 'usync'
            },
            content: [
        ...
        const result = await conn.query(iq)
        const parsed = usyncQuery.parseUSyncQueryResult(result)
        const entry = parsed?.list?.find(item => item.lid || item.id === lidNumber)
        const rawPn = entry?.lid
        if (!rawPn) {
            console.warn(`[LID] USync lid-protocol query(${lidNumber}) tidak balikin PN`, JSON.stringify(parsed?.list))
            return null
        }

        const normalizedPn = rawPn.includes('@') ? decodeJid(rawPn) : decodeJid(`${rawPn}@s.whatsapp.net`)
        if (!normalizedPn.endsWith('@s.whatsapp.net')) return null

        
        try {
            await conn?.signalRepository?.lidMapping?.storeLIDPNMappings?.([{ lid: lidNumber, pn: normalizedPn }])
        } catch (e) {
            console.warn(`[LID] gagal nyimpen hasil USync ke lidMapping cache:`, e.message)
        }

        return normalizedPn
    } catch (e) {
        console.warn(`[LID] USync lid-protocol query(${lidNumber}) error:`, e.message)
        return null
    }
}
...
export async function resolveLidToNumber(lidNumber, conn, chatId) {
    ...
    const fromUSync = await resolveLidViaUSync(lidNumber, conn)
    if (fromUSync) return fromUSync
    ...
    console.warn(`[LID] GAGAL resolve ${lidNumber} lewat semua metode (lidMapping, findUserId, USync lid-protocol, group metadata chat ini, scan semua grup dikenal)`)
    return null
}
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        quoted: {
            get() {
                ...
                const qRaw = JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)) // salinan bersih, tidak ditimpa defineProperties (mis. poll.name)
...
async function convertToAnimatedWebp(buffer, ext) {
    const tmpDir = path.join(process.cwd(), 'data/tmp')
    if (!fs.existsSync(tmpDir)) fs.mkdirSync(tmpDir, { recursive: true })
    const tmp = path.join(tmpDir, `${+new Date()}_${Math.random().toString(36).slice(2)}.${ext}`)
    const out = `${tmp}.webp`
...
async function ensureStickerIsWebp(buffer, stickerMessage) {
    ...
    // Video / animated (gif, mp4, webm) sources need ffmpeg to become an
    // animated webp — jimp hanya menangani gambar raster statis.
    if (type && (/video/i.test(type.mime) || type.ext === 'gif')) {
        return await convertToAnimatedWebp(buffer, type.ext)
    }

    return await jimp.resize(buffer, 512, 512, {
        fit: 'contain',
        background: { r: 0, g: 0, b: 0, alpha: 0 },
        out: 'webp',
    })
...
async function makeStickerTrayWebp(buffer) {
    return await jimp.resize(buffer, 252, 252, { fit: 'cover', out: 'webp' })
}

async function makeBlankStickerTrayWebp() {
    return await jimp.create(252, 252, 0x00000000).getBuffer('image/webp')
}

async function makeStickerThumbnailJpeg(buffer) {
    return await jimp.resize(buffer, 252, 252, { fit: 'cover', out: 'jpeg' })
}
...
async function buildStickerPackMessage(conn, items, { trayBuffer, name, publisher, description } = {}) {
    ...
    for (const item of items) {
        // Stamp author/pack name into each sticker's own EXIF so the
        // watermark shows up per-sticker (not just on the pack listing).
        const stampedBuffer = (!item.isLottie && item.mimetype === 'image/webp')
            ? await stickerAddExif(item.buffer, name || 'Sticker Pack', publisher || '')
            : item.buffer

        const fileName = `${stickerToB64Url(stickerSha256(stampedBuffer))}.${item.ext}`
    ...
    conn.sendStickerPack = async (jid, options = {}, quoted) => {
        ...
        for (const s of stickers) {
            ...
            try {
                ...
                console.error('Skip sticker (gagal convert):', e?.message || e)
            }
        }

        if (!items.length) throw new Error('Semua sticker gagal diproses')
