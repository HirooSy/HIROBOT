...
function isNativeFlowStyle(opts = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        // conn.sendProduct(jid, media, title, description, quoted, { businessOwnerJid, ...options })
        // - media: single item or an array. Each item can be anything
        //   conn.getFile() already accepts (Buffer, URL string, local path,
        //   data: URI) since that's the same normalization sendFile uses.
        // - Array input sends one productMessage per image (WhatsApp's
        //   ProductSnapshot proto only carries one productImage per
        //   message - productImageCount just reports the total series
        //   length on each one, it doesn't embed a gallery in one bubble).
        // - businessOwnerJid defaults to the bot's own jid (this.user.jid) -
        //   '0@s.whatsapp.net' was tried first as a generic placeholder but
        //   WhatsApp actually rejects/breaks the product card with that
        //   value, so it has to be a real, resolvable account.
        //
        // Product images go through prepareWAMessageMedia({image:...}) just
        // like a normal chat image (mediaType 'image', endpoint /mms/image).
        // A 'product-catalog-image' upload (endpoint /product/image) was
        // tried first, mirroring Baileys' own uploadingNecessaryImages -
        // but that endpoint returned "Media upload failed on all hosts",
        // meaning it needs business/catalog account context this bot
        // doesn't have. That path is for the separate Business Catalog
        // REST API (conn.productCreate etc, plain {url} results, no
        // ImageMessage proto at all) - not for embedding an image inside a
        // productMessage sent in a normal chat, which is what this does.
        sendProduct: {
