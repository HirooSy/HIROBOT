...
async function stickerConvert(img, url, categories = ['']) {
    ...
    const fluent_ffmpeg = (await import('fluent-ffmpeg')).default
...
const VOIP_AUTH_DIR = 'data/sessions/caller.db'
const VOIP_WORKER_PATH = path.join(__dirname, '../package/voip', 'modules', 'worker.js')
        ...
        proc.on('message', (msg) => {
            switch (msg.type) {
                ...
                case 'call_connected':
                    ...
                    scheduleSafetyTimeout((durationMs ?? 120_000) + 30_000)
                    break
                case 'ended':
                ...
                case 'error':
                    ...
                    _voipCleanup()
                    break
                case 'unknown_call_event':
                    // DIAGNOSTIC passthrough — see lib/package/voip/index.js
                    // #handleCallEvent and lib/package/voip/modules/worker.js.
                    callEmitter.emit('unknownCallEvent', { eventType: msg.eventType, eventData: msg.eventData })
                    break
            }
        })
...
export async function voipPair(conn, customPairingCode) {
    ...
    if (fs.existsSync(path.join(process.cwd(), VOIP_AUTH_DIR, 'creds.json'))) {
