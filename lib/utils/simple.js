...
class AIRich extends BaseBuilder {
	...
	addHtml(...items) {
		...
		if (!items.every((item) => Array.isArray(item) && typeof item[0] === 'string')) {
			throw new TypeError('addHtml() expects either a single HTML string, or one or more [html, title] pairs');
		}
			...
			botForwardedMessage: {
				message: {
					richResponseMessage: {
						...
						unifiedResponse: {
							data: includesUnifiedResponse ? Buffer.from(JSON.stringify(responseData)).toString('base64') : '',
						},
						contextInfo: {
							...forward,
							...qObj,
