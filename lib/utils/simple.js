...
// The a2ui Text component inside bloksWidget renders with a plain system
// font that has no color-emoji glyphs, so full emoji (❤️, ⭐, ...) show up
// as tofu boxes there even though they render fine in a normal WA message.
// ICON_ASCII_MAP is the safe default: plain ASCII, guaranteed to render.
// ICON_SYMBOL_MAP is a lighter-weight alternative using basic single-
// codepoint Unicode symbols (no variation selectors / no color-emoji
// block) — more likely to render than full emoji, but still unverified on
// every client, so it's opt-in via item.style = 'symbol'.
const ICON_ASCII_MAP = {
...
function nfBuildWidgetItem(item = {}, id, components) {
    if (item.image) {
        ...
        if (item.native) {
            ...
            const map = item.style === 'symbol' ? ICON_SYMBOL_MAP : ICON_ASCII_MAP
            const fallback = item.style === 'symbol' ? '•' : `[${iconName}]`
            components.push({ id, component: 'Text', text: map[iconName] || fallback, variant: item.variant || 'body' })
        }
    } else if (item.video) {
        ...
        const childIds = item.row.map((sub, j) => nfBuildWidgetItem(sub, `${id}_${j}`, components)).filter(Boolean)
        components.push({ id, component: 'Row', children: childIds, justify: item.justify || 'start', align: item.align || 'stretch' })
    } else if (item.column) {
        const childIds = item.column.map((sub, j) => nfBuildWidgetItem(sub, `${id}_${j}`, components)).filter(Boolean)
        components.push({ id, component: 'Column', children: childIds, justify: item.justify || 'start', align: item.align || 'stretch' })
    } else if (item.list) {
        const childIds = item.list.map((sub, j) => nfBuildWidgetItem(sub, `${id}_${j}`, components)).filter(Boolean)
        components.push({ id, component: 'List', children: childIds, direction: item.direction || 'vertical', align: item.align || 'stretch' })
    } else {
        return null
    }
    return id
}
function nfWidgetItemFallback(item = {}) {
    if (item.image) return '[foto]'
    if (item.text) return String(item.text)
    if (item.icon || item.name) return ICON_ASCII_MAP[item.icon || item.name] || `[${item.icon || item.name}]`
    ...
    if (item.datetime) return `${item.label || 'tanggal'}: ${item.value || ''}`
    if (item.row) return item.row.map(nfWidgetItemFallback).filter(Boolean).join(' | ')
    if (item.column || item.list) return (item.column || item.list).map(nfWidgetItemFallback).filter(Boolean).join('\n')
    return ''
}
function nfBuildBloksWidget(widget = {}) {
    ...
    const children = items
        .map((item, i) => nfBuildWidgetItem(item, `item_${i}`, components))
        .filter(Boolean)
    ...
    const autoFallback = items.map(nfWidgetItemFallback).filter(Boolean).join('\n')
    return {
        uuid,
        data,
        type: 'im_a2ui',
        fallback: fallback || autoFallback || 'Cannot load Widget A2UI'
    }
}
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        quoted: {
            get() {
                ...
                const qRaw = JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q))
