...
const authFolder = path.dirname(global.settings.connection.main.file)
const authDbFile = 'data/sessions/main.db'
...
let [
  ...
  useSQLiteAuthState(authDbFile)
])

if (isCredsExist && !authState.state.creds?.registered) {
  console.debug('- sqliteauth -', 'legacy creds.json found, migrating into SQLite session...')
  const legacyAuth = await useMultiFileAuthState(authFolder)
  authState.state.creds = legacyAuth.state.creds
  await authState.saveCreds()
  console.debug('- sqliteauth -', 'migration complete')
}
...
if (Helper.opts['singleauth'] || Helper.opts['singleauthstate']) {
  if (!isCredsExist && isAuthSingleFileExist) {
    ...
    const tempMultiAuth = await useMultiFileAuthState(authFolder)
    await single2multi(authFile, authFolder, tempMultiAuth)
    console.debug('- singleauth -', 'compiled successfully, migrating into SQLite session...')
    authState.state.creds = tempMultiAuth.state.creds
    await authState.saveCreds()
