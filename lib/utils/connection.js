...
// Serializes concurrent reads/writes to the same signal-key row (type+id)
// in the SQLite auth store. Baileys itself (via makeCacheableSignalKeyStore)
// does not serialize access to the underlying keys.get/keys.set - if two
// events for the same session/sender-key/pre-key land close together, both
// can read the old row, mutate independently, and write back, silently
// discarding one of the two updates (a lost-update race). That is a common
// cause of "Bad MAC" / "No matching sessions" errors once Baileys is backed
// by a real DB. Wrapping keys.get/keys.set below with per-row locking fixes
// this without touching anything inside the baileys package itself.
class StoreLock {
  #chains = new Map()
  ...
  // Locks several keys at once, serialized in a fixed (sorted) order so
  // overlapping batches can never deadlock on each other.
  runMany(keys, task) {
    ...
    state: {
      ...
      keys: {
        get: async (type, ids) => {
          ...
          // Lock each row individually so a concurrent set() for the same
          // id can't sneak in between our reads and produce a torn batch.
...
// Forces a reload/reconnect of the given connection without waiting for a
// real 'connection.update' close event from Baileys. Needed for cases where
// Baileys throws internally (uncaughtException/unhandledRejection deep in
// its socket/decrypt code) - those errors never surface as a proper
// 'close' event, so the normal connectionUpdate() reconnect path is never
// triggered and the socket is left stuck. This reuses the same reload()
// used by connectionUpdate, and the same isReconnecting guard, so it can't
// race with a legitimate reconnect already in progress.
async function forceReconnect(conn, opts, reason = 'forced') {
  ...
  try {
    ...
    await reload(conn, true, opts).catch(err => console.error(chalk.red('Reload error:'), safeErr(err)))
    if (global?.timestamp) global.timestamp.connect = new Date()
    pairingCodeRequested = false
    return true
  } finally {
...
export { conn, conns, logger, forceReconnect }