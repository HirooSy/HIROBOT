...
async function loadVoip() {
  ...
  loading = (async () => {
    ...
    try {
      // 'caller' is the package.json alias for npm:@roamhq/wrtc — check under
      // that folder name, but install using the full alias spec so npm
      // actually resolves and downloads @roamhq/wrtc under it.
      installIfMissing('caller', 'caller@npm:@roamhq/wrtc', projectRoot)
    } catch (e) {
