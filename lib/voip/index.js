...
import { VideoFeeder } from "./video-feeder.js";

// Hard resource ceiling for video calls on this ~1GB-memory host — see the
// comment on #applyVideoEncoderParams for why these are capped rather than
// left to WASM's adaptive controller.
const MAX_VIDEO_WIDTH = 160;
const MAX_VIDEO_HEIGHT = 120;
const MAX_VIDEO_FPS = 8;
...
export class ActiveCall extends EventEmitter {
    ...
    _updateState = (state, callResult) => {
        ...
        if (state === CallState.PreacceptReceived)
            ...
            // Active almost always means the peer rejected/declined the
            // call rather than a normal hangup. Once the call HAS been
            // Active, non-zero call_result values show up as routine noise
            // alongside otherwise-healthy state transitions (confirmed
            // repeatedly in call_info logs) — checking callResult before
            // state, or on every state transition regardless of phase (as
            // an earlier version of this function did), silently swallows
            // the ringing/connected emits themselves, since it returns
            // before ever reaching the state checks below.
...
export class VoipClient {
    ...
    #handleCallEvent = (eventType, eventData) => {
        if (eventType === 16 && eventData) {
            ...
            // WASM's adaptive bitrate controller pushing new encoder
            // targets (fires frequently — bitrate/fps/resolution all
            // change live based on network conditions). Forward to the
            // active VideoFeeder so ffmpeg's output actually tracks what
            // WhatsApp is asking for instead of encoding at a fixed
            // config the whole call.
            try {
            ...
            // Lighter-weight fps-only hint that precedes a full 164 update
            // in the logs — handled by the same target application logic.
            try {
                const params = JSON.parse(eventData);
                this.#applyVideoEncoderParams(params);
            }
    ...
    // WASM's adaptive bitrate controller (network-condition-driven) sends
    // updated encode targets throughout the call — width/height/fps/bitrate
    // all can change live. ffmpeg doesn't support changing encode
    // parameters on a running process, so this restarts VideoFeeder with
    // the new targets rather than attempting to reconfigure it in place.
    // Skips the restart if nothing meaningful actually changed, since these
    // events fire frequently and a restart means a fresh keyframe + brief
    // encode gap each time.
    //
    // Hard-capped at MAX_VIDEO_WIDTH/HEIGHT/FPS regardless of what WASM
    // requests — this host's ~1GB memory budget has already been observed
    // to OOM-kill the whole process running a second ffmpeg encode at
    // 320x240, so upgrading resolution mid-call on WASM's request (which it
    // will do automatically as it detects good network conditions) risks
    // crashing an otherwise-healthy call. Capping here is a deliberate
    // stability tradeoff, not an oversight.
    #applyVideoEncoderParams = (params) => {
        if (!this.#videoFeeder) return;
        ...
        const requestKeyframe = !!params.request_keyframe;
        const unchanged = newWidth === this.#videoWidth && newHeight === this.#videoHeight && newFps === this.#videoFps;
        if (unchanged) {
            if (requestKeyframe) {
                this.#videoFeeder.stop();
                this.#handleVideoCaptureStart({ streamId: this.#videoStreamId, width: this.#videoWidth, height: this.#videoHeight, fps: this.#videoFps });
            }
            return;
        }
        ...
        this.#videoFeeder.stop();
        this.#handleVideoCaptureStart({ streamId: this.#videoStreamId, width: newWidth, height: newHeight, fps: newFps });
    };
}
