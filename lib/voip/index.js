...
/**
 * SQLite-backed replacement for Baileys' useMultiFileAuthState, storing the
 * VOIP-linked device's creds + signal keys in a single .data (SQLite) file
 * instead of a folder of loose .json files.
 */
const KEY_MAP = {
    'pre-key': 'preKeys',
    'session': 'sessions',
...
export class VoipClient {
    ...
    call = async (phoneNumber, opts = {}) => {
        ...
        // Titik referensi tunggal untuk audio & video: keduanya menunggu
        // event 'connected' ini (bukan saling menunggu satu sama lain,
        // yang terbukti rawan bug timing/delay tak terduga), supaya
        // sama-sama mulai dari momen call benar-benar aktif.
    ...
    #handleAudioCaptureStart = () => {
        ...
        // Audio & video sama-sama menunggu event 'connected' dari call
        // (lihat #callConnectedPromise), bukan saling menunggu satu sama
        // lain — pendekatan saling-tunggu sebelumnya rawan bug timing
        // (salah satu bisa tertahan lama kalau yang lain lambat siap).
        const feederAtStart = this.#feeder;
        const waitStartedAt = Date.now();
        (this.#callConnectedPromise ?? Promise.resolve()).then(() => {
            console.error(`[VOIP] audio starting ${Date.now() - waitStartedAt}ms after call connected`);
    ...
    #handleVideoCaptureStart = (config) => {
        ...
        this.#videoRestartChain = this.#videoRestartChain.then(async () => {
            ...
            (this.#callConnectedPromise ?? Promise.resolve()).then(() => {
                console.error(`[VOIP] video starting ${Date.now() - videoWaitStartedAt}ms after call connected`);
