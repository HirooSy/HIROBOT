...
export class VoipClient {
    ...
    connect = async () => {
        ...
        this.#signaling.setGroupCallDetectedHandler((callId) => {
            if (this.#activeCall?.callId === callId) {
                console.error(`[ VOIP ] Participant added/removed for callId=${callId} — forwarding to WASM as usual; bot stays on the call until it actually ends (all participants leave or someone terminates it).`);
            }
        });
        ...
        this.#onCbCall = (node) => {
            ...
            if (this.#activeCall && this.#activeCall.state !== CallState.Idle && this.#activeCall.state !== CallState.Ending) {
                ...
                const voipChild = node.content?.[0];
                if (voipChild?.tag === "offer") {
                    const incomingCallId = String(voipChild.attrs?.["call-id"] ?? voipChild.attrs?.call_id ?? "");
                    if (incomingCallId && incomingCallId !== this.#activeCall.callId) {
                        this.#signaling.rejectIncomingCall(node, "busy");
                        return;
                    }
                }
            }
