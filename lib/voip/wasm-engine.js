...
export class WasmEngine {
    ...
    constructor(config = {}) {
        ...
        const wasmPath = config.wasmPath
            ...
            : path.join(basePath, "modules", "whatsapp.wasm");
        this.#config = {
            ...config,
            wasmPath,
    ...
    startCall = (options) => {
        ...
        try {
            // Restored to match the upstream baileys-caller source exactly
            // (src/wasm-engine.mts): try the 8-parameter signature first
            // (peerJid, deviceJids, callId, isVideo, peerPn, isLidCall,
            // isFromDialer, tcToken), falling back to 7 params only on a
            // genuine BindingError. An earlier "fix" here incorrectly
            // dropped isLidCall based on a mismatched comparison against
            // WhatsApp Web's separate worker-RPC signature — that broke
            // call setup outright (reject before ringing) because this
            // WASM build's #instance.startVoipCall binding genuinely
            // expects 8 parameters.
            try {
                const result = this.#instance.startVoipCall(peerJid, peers, options.callId, options.isVideo, options.peerPn, isLidCall, isFromDialer, tcToken);
                console.error(`[ VOIP-DIAG ] startVoipCall 8-param path SUCCEEDED, result=${JSON.stringify(result)}`);
                return result;
            }
            catch (error) {
                console.error(`[ VOIP-DIAG ] startVoipCall 8-param path THREW: name=${error?.name} message=${error?.message || error}`);
                if (error?.name !== "BindingError")
                    throw error;
                const result = this.#instance.startVoipCall(peerJid, peers, options.callId, options.isVideo, options.peerPn, isFromDialer, tcToken);
                console.error(`[ VOIP-DIAG ] startVoipCall 7-param fallback SUCCEEDED, result=${JSON.stringify(result)}`);
                return result;
            }
        }
        finally {
    ...
    // Feeds one already-encoded H.264 Annex-B access unit to the WASM engine.
    // Signature reverse-engineered from the official client's own JS->WASM
    // call site (WAWebVoipWebCodecsEncoder's handleEncodedChunk, "Ie" in the
    // minified worker-modules bundle):
    //   onEncodedVideoDataFromJsForStream(streamId, ptr, byteLength, width,
    //     height, timestampMs, isKeyFrame, latencyMs, orientation)
    // Unlike sendAudioData's persistent capture buffer, each access unit is
    // variable-length, so this mallocs fresh per call and always frees in
    // `finally` — matching the reference implementation exactly, since a
    // missed free here would leak WASM heap once per video frame (tens of
    // times per second).
    sendVideoData = (streamId, accessUnit, width, height, timestampMs, isKeyFrame, latencyMs, orientation) => {
        this.#ensureInitialized();
        if (!accessUnit || accessUnit.length === 0)
            return 0;
        if (typeof this.#instance.onEncodedVideoDataFromJsForStream !== "function")
            return 0;
        const heapU8 = this.#instance.GROWABLE_HEAP_U8?.() ?? this.#instance.HEAPU8;
        if (!heapU8)
            return 0;
        const ptr = this.#instance._malloc(accessUnit.length);
        if (!ptr)
            return 0;
        try {
            heapU8.set(accessUnit, ptr);
            return this.#instance.onEncodedVideoDataFromJsForStream(streamId, ptr, accessUnit.length, width, height, timestampMs, isKeyFrame, latencyMs, orientation);
        }
        catch (e) {
            console.error('[ VOIP ] sendVideoData failed:', e?.message || e);
            return 0;
        }
        finally {
    ...
    #applyDefaultAbProps = () => {
        ...
        const intProps = {
            ...
            opus_max_bandwidth: 1104,
        };
        const boolProps = {
            enable_av_downgrade: false,
            enable_new_user_action_stanza_for_raise_hand_sender: false,
            // EXPERIMENTAL: video support. These two flags exist in the WASM's
            // config surface but were left disabled in the original library.
            // "Passthrough" suggests raw H.264 bitstream in/out rather than
            // WASM-internal codec work, which would let @roamhq/wrtc's native
            // WebRTC H.264 encoder/decoder do the actual codec work instead of
            // requiring us to reimplement one. Untested — may be a no-op if the
            // underlying WASM build was stripped of the actual video codepath.
            enable_webcodec_video_encode: true,
            ...
            enable_passthrough_video_decoder: true,
        };
    ...
    #registerGlobalCallbacks = () => {
        ...
        if (callbacks.onVideoCaptureStart) {
            _a.registerGlobalCallbackListener("startVideoCaptureJS", (data) => {
                callbacks.onVideoCaptureStart({
                    streamId: data?.stream_id ?? data?.streamId ?? 0,
                    width: data?.width, height: data?.height, fps: data?.max_fps ?? data?.fps,
                });
            });
        }
        if (callbacks.onVideoCaptureStop) {
    ...
    #createVMContext = (memory) => {
        ...
        const wasmCallbacks = {
            ...
            stopVideoCaptureJS: () => { callbacks.onVideoCaptureStop?.(); return 0; },
