...
export class AudioFeeder {
    ...
    start = () => {
        ...
        this.#proc.stderr.on("data", (chunk) => {
            console.error(`[AudioFeeder] ${chunk.toString().trim()}`);
        });
        this.#proc.on("error", (err) => {
            console.error(`[AudioFeeder] ffmpeg spawn error: ${err?.message || err}`);
        });
        this.#proc.on("exit", (code) => {
            const cleanExit = code === 0 || code === null;
            if (!cleanExit) {
                console.error(`[AudioFeeder] ffmpeg exited with code=${code} (source=${this.source})`);
            }
            this.#proc = null;
            if (cleanExit && this.source && this.source !== "silence" && !this.#stoppedManually) {
                this.onFinished?.();
            }
        });
        this.#nextEmitAtMs = 0;
