...
export class RelayRtcTransport {
    ...
    #connect = async (connection) => {
        ...
        if (typeof RTCPeerConnection !== "function") {
            throw new Error("RTCPeerConnection unavailable from 'caller' (aliased to @roamhq/wrtc)");
        }
        ...
        try {
            ...
            if (e?.name === "InvalidStateError" || /signalingState.*closed/i.test(e?.message ?? "")) {
                connection.state = "failed";
                return;
            }
            throw e;
    ...
    #loadWrtc = () => {
        this.#wrtcPromise ??= import("caller").then((module) => (module.default ?? module));
