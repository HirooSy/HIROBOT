...
let _privacyTokensUnsupportedWarned = false;
...
export class SignalingBridge {
    ...
    requestTcToken = async (jid) => {
        ...
        if (cached?.length)
            return cached;
        try {
            if (typeof this.#sock.getPrivacyTokens !== "function") {
                if (!_privacyTokensUnsupportedWarned) {
                    console.warn(`[ VOIP ] sock.getPrivacyTokens is not available in this Baileys version — will silently fall back to stale cached tokens for the rest of this session.`);
                    _privacyTokensUnsupportedWarned = true;
                }
            ...
            const tokenNodes = tokensNode
                ...
                : [];
            console.log(`[ VOIP ] requestTcToken for ${userJid}: getPrivacyTokens returned ${tokenNodes.length} token node(s)`);
            for (const tokenNode of tokenNodes) {
                ...
                if (content instanceof Uint8Array && content.length > 0) {
                    ...
                    console.log(`[ VOIP ] requestTcToken for ${userJid}: got fresh token (len=${token.length}, ts=${tokenNode.attrs.t})`);
                    return token;
                }
            }
            console.log(`[ VOIP ] requestTcToken for ${userJid}: no matching token in response`);
        }
        catch (e) {
            console.error(`[ VOIP ] requestTcToken for ${userJid} failed:`, e?.message || e);
        }
        return this.#getTcToken(userJid, true);
    };
    ...
    #getTcToken = async (jid, allowStale = false) => {
        const userJid = this.#toBareJid(jid);
        const MAX_TOKEN_AGE_SEC = 6 * 60 * 60; // TC tokens observed to go stale after a few hours; be conservative
        ...
        const warnStaleOnce = (source, timestamp) => {
            ...
            console.log(`[ VOIP ] ${source} tcToken for ${userJid} is stale (ts=${timestamp})${allowStale ? ', using anyway (no fresher option available)' : ', ignoring'}.`);
        };
