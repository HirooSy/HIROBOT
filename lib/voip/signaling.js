...
export class SignalingBridge {
    ...
    constructor(config) {
        ...
        this.#onSignalingError = config.onSignalingError;
    }
    ...
    sendSignaling = (peerJid, callId, xmlPayload) => {
        this.#outgoingSignalingQueue = this.#outgoingSignalingQueue
            ...
            .catch((e) => {
                // sendSignaling is invoked fire-and-forget from a synchronous
                // WASM callback, so a thrown error here has nowhere to
                // propagate to on its own — it used to vanish silently,
                // leaving the call to time out as MISSED with zero
                // indication of why (e.g. callKey encryption failing for
                // every device). Surface it via onSignalingError so the
                // active call can end with a real reason instead.
                console.error('[ VOIP ] Outbound signaling failed:', e?.message || e);
                this.#onSignalingError?.(callId, e);
            });
    };
    ...
    requestTcToken = async (jid) => {
        ...
        if (cached?.length)
            return cached;
        try {
            const { getBinaryNodeChild, getAllBinaryNodeChildren, jidNormalizedUser } = this.#baileys;
            const timestamp = String(Math.floor(Date.now() / 1000));

            // The real Baileys socket method is `issuePrivacyTokens` (NOT
            // `getPrivacyTokens` — that name doesn't exist anywhere in the
            // actual Baileys source). Prefer it when available since it's
            // guaranteed to build the exact stanza WhatsApp expects.
            let response;
            if (typeof this.#sock.issuePrivacyTokens === "function") {
                response = await this.#sock.issuePrivacyTokens([userJid], Number(timestamp));
            }
            else {
                // Manual fallback, mirroring Baileys' messages-send.ts
                // issuePrivacyTokens() verbatim: type MUST be "set", not "get" —
                // this is an issuance request, not a fetch.
                response = await this.#sock.query({
                    tag: "iq",
                    ...
                    content: [
                        {
                            tag: "tokens",
                            attrs: {},
                            content: [userJid].map((j) => ({
                                tag: "token",
                                attrs: { jid: jidNormalizedUser(j), t: timestamp, type: "trusted_contact" },
                            })),
                        },
                    ],
                });
            ...
            const tokenNodes = tokensNode
                ...
                : [];
            console.log(`[ VOIP ] requestTcToken for ${userJid}: issuePrivacyTokens returned ${tokenNodes.length} token node(s)`);
            for (const tokenNode of tokenNodes) {
                // Per Baileys' storeTcTokensFromIqResult: only 'trusted_contact'
                // type tokens are valid, and in some notification paths the
                // node's own jid attr is OUR device jid, not the peer's — so
                // we trust the jid we requested for, not necessarily the
                // node's attrs.jid, when there's exactly one token back.
                ...
                if (content instanceof Uint8Array && content.length > 0) {
                    ...
                    await this.#sock.authState.keys.set({
                        tctoken: { [userJid]: { token, timestamp: String(tokenNode.attrs.t ?? timestamp) } },
                    });
                    console.log(`[ VOIP ] requestTcToken for ${userJid}: got fresh token (len=${token.length})`);
                    return token;
                }
            }
            console.log(`[ VOIP ] requestTcToken for ${userJid}: no matching token node in response`);
        }
        catch (e) {
            console.error(`[ VOIP ] requestTcToken manual query for ${userJid} failed:`, e?.message || e);
        }
        return this.#getTcToken(userJid, true);
    };
    ensureTcToken = async (...jids) => {
        ...
        for (const jid of uniqueJids) {
            const cached = await this.#getTcToken(jid, false);
            if (cached?.length)
                return cached;
        }
        for (const jid of uniqueJids) {
        ...
        // Last resort: no fresh token anywhere and fetching didn't help
        // (e.g. getPrivacyTokens unavailable) — use the stalest cached
        // token we have rather than proceeding with none at all.
        for (const jid of uniqueJids) {
            const stale = await this.#getTcToken(jid, true);
            if (stale?.length)
                return stale;
        }
        return undefined;
    ...
    issueTcToken = async (jid) => {
        ...
        try {
            const response = await this.#sock.query({
                tag: "iq",
                attrs: {
            ...
            // The IQ response echoes back a fresh token for this jid (mirrors
            // Baileys' issuePrivacyTokens -> storeTcTokensFromIqResult flow).
            // Previously this response was discarded entirely, so the token
            // request "succeeded" but nothing was ever actually saved.
            const { getBinaryNodeChild, getAllBinaryNodeChildren } = this.#baileys;
            const tokensNode = getBinaryNodeChild(response, "tokens");
            const tokenNodes = tokensNode
                ? getAllBinaryNodeChildren(tokensNode).filter((c) => c.tag === "token")
                : [];
            console.log(`[ VOIP ] issueTcToken for ${userJid}: response had ${tokenNodes.length} token node(s)`);
            for (const tokenNode of tokenNodes) {
                if (tokenNode.attrs.type !== "trusted_contact")
                    continue;
                const content = tokenNode.content;
                if (content instanceof Uint8Array && content.length > 0) {
                    ...
                    this.#rememberTcToken(userJid, content, String(tokenNode.attrs.t ?? issuedAt));
                    console.log(`[ VOIP ] issueTcToken for ${userJid}: saved fresh token (len=${content.length})`);
                }
            }
            return true;
        }
        catch (e) {
            console.error(`[ VOIP ] issueTcToken for ${userJid} failed:`, e?.message || e);
            return false;
        }
    };
        ...
        if (destination) {
            ...
            for (const destNode of destinations) {
                ...
                try {
                    ...
                    // Previously: a single device's callKey-encryption failure
                    // silently stripped the enc node from EVERY destination and
                    // still sent the offer — WhatsApp then had no valid media
                    // key for any device, so the call looked "sent" but could
                    // never actually establish media, timing out as MISSED
                    // with no visible error anywhere. Now: drop only the
                    // failing device (the others may still have a valid key),
                    // and if every device fails, abort the call outright with
                    // a clear error instead of sending a broken offer.
                    console.error(`[ VOIP ] callKey encryption failed for device ${targetJid}:`, e?.message || e);
                    failedDevices.push(targetJid);
                    removeNodeChildrenByTag(destNode, "enc");
                }
            }
            if (failedDevices.length === destinations.length) {
                throw new Error(`callKey encryption failed for all ${destinations.length} device(s), cannot place call`);
            }
    ...
    #staleWarned = new Set();
    #getTcToken = async (jid, allowStale = false) => {
        const userJid = this.#toBareJid(jid);
        // Mirrors Baileys' own isTcTokenExpired(): 4 rolling 7-day buckets,
        // i.e. tokens are valid up to ~28 days old, not just a few hours.
        // (An earlier, much stricter guess of 6 hours here was wrong and was
        // discarding tokens WhatsApp still considers valid.)
        const MAX_TOKEN_AGE_SEC = 28 * 24 * 60 * 60;
        const isFresh = (timestamp) => {
            const ts = Number(timestamp);
            if (!ts) return false;
            return (Date.now() / 1000 - ts) < MAX_TOKEN_AGE_SEC;
        };
        const warnStaleOnce = (source, timestamp) => {
            const key = `${userJid}:${source}`;
            if (this.#staleWarned.has(key)) return;
            this.#staleWarned.add(key);
            console.log(`[ VOIP ] ${source} tcToken for ${userJid} is stale (ts=${timestamp}).`);
        };
        const observedEntry = this.#observedTcTokens.get(userJid);
        if (observedEntry?.token?.length) {
            if (isFresh(observedEntry.timestamp) || allowStale) {
                if (!isFresh(observedEntry.timestamp)) warnStaleOnce('Cached', observedEntry.timestamp);
                return Buffer.from(observedEntry.token);
            }
            warnStaleOnce('Cached', observedEntry.timestamp);
        }
        try {
            const data = await this.#sock.authState.keys.get("tctoken", [userJid]);
            const entry = data[userJid];
            if (entry?.token?.length) {
                if (isFresh(entry.timestamp)) {
                    this.#rememberTcToken(userJid, entry.token, entry.timestamp);
                    return entry.token;
                }
                warnStaleOnce('Persisted', entry.timestamp);
                if (allowStale) return entry.token;
            }
        }
        catch { }
        return undefined;
