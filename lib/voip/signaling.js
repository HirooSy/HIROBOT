...
export class SignalingBridge {
    ...
    ensureTcToken = async (...jids) => {
        ...
        // Deliberately NOT falling back to a stale cached token here.
        // Empirical finding from a sibling project (AstraCalls) that builds
        // WhatsApp call stanzas manually and logged the raw protocol
        // exchange: attaching a stale tctoken makes the recipient silently
        // DROP the offer — even numbers that would otherwise ring stop
        // ringing. No token at all rings normally (reaches preaccept). An
        // earlier version of this function fell back to a stale token as a
        // "better than nothing" last resort; that was backwards and is very
        // likely the actual cause of our repeated call_result=4 rejections.
        console.log('[ VOIP ] No fresh tcToken available — proceeding WITHOUT a token (a stale one is worse than none).');
        return undefined;
    };
