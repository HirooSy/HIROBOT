...
export class VideoFeeder {
    ...
    start = () => {
        ...
        this.#proc = spawn("ffmpeg", ffmpegArgs);
        ...
        this.#proc.on("exit", (code) => {
            ...
            else if (!this.#sawFirstFrame) {
                // Failed before producing even one frame — almost certainly
                // a source problem (bad codec, corrupt file, ffmpeg build
                // missing a needed encoder/filter) rather than a transient
                // hiccup. Report it distinctly so the caller can stop
                // retrying instead of looping restart attempts that will
                // fail identically every time.
                this.onError?.(new Error(`ffmpeg exited with code=${code} before producing any frames`));
            }
            else {
                // Produced at least one good frame before dying mid-stream —
                // more likely transient, so let the normal onFinished/retry
                // path handle it rather than treating it as a hard failure.
                this.onFinished?.();
            }
        });
        this.#scheduleNext();
