...
async function runCall({ phoneNumber, audioSource, videoSource, durationMs, isVideo }) {
  ...
  try {
    ...
    call.on('connected', () => {
      ...
      // Extend the worker-local safety net to cover the call's actual
      // duration now that it's live, mirroring what the parent process
      // does with its own timer in simple.js.
...
async function main() {
  ...
  // Worker-local safety net, independent of the parent's own safety timer
  // in simple.js — this guards against the worker process itself hanging
  // (e.g. stuck in setup before a call object even exists) rather than
  // against the call's IPC messages not reaching the parent. Like the
  // parent's timer, this must be extended once the call actually connects,
  // not left at the short setup-phase cap for the call's entire duration —
  // otherwise a perfectly healthy, ongoing call gets force-killed here
  // regardless of what the parent's own timer allows.
