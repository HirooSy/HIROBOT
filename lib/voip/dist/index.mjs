...
export class VoipClient {
     ...
     *  If config.existingSocket is provided (an already-open Baileys socket),
     *  it is reused as-is and no new device link / QR flow is triggered. */
    connect = async () => {
        if (this.#config.existingSocket) {
            ...
            await new Promise((resolveOpen, rejectOpen) => {
                ...
                const connectSocket = () => {
                    ...
                    // Pairing-code flow: request it once the socket is up, only if
                    // we are not already registered (no creds yet) and the caller
                    // asked for it via config.pairingCode.
    ...
    #handleAudioCaptureInit = (config) => {
        console.log('[ VOIP ] #handleAudioCaptureInit called with config:', JSON.stringify(config));
        if (!this.#engine)
            return;
        ...
        console.log('[ VOIP ] capturePtr allocated:', this.#capturePtr, 'chunkBytes:', this.#captureChunkBytes);
    };
    #handleAudioCaptureStart = () => {
        console.log('[ VOIP ] #handleAudioCaptureStart called. engine=', !!this.#engine, 'capturePtr=', this.#capturePtr);
        if (!this.#engine || !this.#capturePtr)
            return;
        const audioSource = this.#activeCall?._audioSource ?? "silence";
        console.log('[ VOIP ] Starting AudioFeeder with source:', audioSource);
        let _chunkCount = 0;
        this.#feeder = new AudioFeeder(this.#captureSampleRate, this.#captureChannels, this.#captureFramesPerChunk, (chunk) => {
            ...
            if (_chunkCount === 1 || _chunkCount % 100 === 0) {
                console.log(`[ VOIP ] audio chunk #${_chunkCount} sent to WASM, engine=${!!this.#engine}, capturePtr=${this.#capturePtr}`);
            }
            if (this.#engine && this.#capturePtr)
                this.#engine.sendAudioData(chunk, this.#capturePtr);
        }, audioSource);
        this.#feeder.start();
        console.log('[ VOIP ] AudioFeeder.start() called, sampleRate=', this.#captureSampleRate, 'channels=', this.#captureChannels);
    };
