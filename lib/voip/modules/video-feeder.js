...
const FFMPEG_THREADS = Math.max(1, Math.min(2, os.cpus()?.length || 1));
...
export class VideoFeeder {
    ...
    start = () => {
        ...
        const keyframeIntervalFrames = this.targetFps * 4;
        const ffmpegArgs = [
            "-hide_banner",
            "-loglevel", "warning",
            ...
            "-bf", "0",
            "-pix_fmt", "yuv420p",
            "-x264-params", `threads=${FFMPEG_THREADS}:keyint=${keyframeIntervalFrames}:min-keyint=${keyframeIntervalFrames}:scenecut=0`,
            "-f", "h264",
            "pipe:1",
        ];
        ...
        this.#proc.on("exit", (code, signal) => {
            ...
            if (this.#stoppedManually) {
                ...
                if (!cleanExit && !this.#sawFirstFrame) {
                    this.onError?.(new Error(`ffmpeg killed (stopped) with code=${code} before producing any frames`));
                }
                return;
            }
            if (cleanExit) {
