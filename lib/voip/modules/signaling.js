...
export class SignalingBridge {
    ...
    #onGroupCallDetected = null;
    constructor(config) {
        this.#sock = config.sock;
    }

    setGroupCallDetectedHandler = (callback) => {
        this.#onGroupCallDetected = callback;
    };
    ...
    rejectIncomingCall = async (node, reason = "busy") => {
        try {
            ...
            if (!incomingCallId || !fromJid) return;
            await this.#sock.sendNode({
                tag: "call",
                attrs: { to: fromJid, id: this.#sock.generateMessageTag() },
                content: [{
                    tag: "reject",
                    attrs: { "call-id": incomingCallId, "call-creator": fromJid, count: "0", reason },
                }],
            });
        }
        catch { }
    ...
    #doProcessIncomingCall = async (node, voip, activeCallId) => {
        ...
        if (activeCallId && incomingCallId && incomingCallId !== activeCallId) {
            if (voipChild.tag === "offer") {
                this.rejectIncomingCall(node, "busy");
            }
            return;
        }
        ...
        if ((usableNode.tag === "add" || usableNode.tag === "remove") && callIdForRouting) {
            this.#onGroupCallDetected?.(callIdForRouting);
        }
