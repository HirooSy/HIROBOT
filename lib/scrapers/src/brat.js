...
// pureimage & jimp are heavy (decoders, font parser, etc). Loading them at
// module top-level means they get pulled into memory as soon as this file
// is imported by the scraper loader at boot — even if `.brat` is never
// used. Deferring to a dynamic import means they only load the first time
// someone actually triggers a brat generation.
let pureimage = null;
let Jimp = null;
let _imageLibsPromise = null;

function loadImageLibs() {
    if (!_imageLibsPromise) {
        _imageLibsPromise = Promise.all([
            import('pureimage'),
            import('jimp'),
        ]).then(([pureimageMod, jimpMod]) => {
            pureimage = pureimageMod;
            Jimp = jimpMod.Jimp;
        });
    }
    return _imageLibsPromise;
}
