        ...
        for (const wi of lineWordIdxs) {
            for (const s of wordSegs[wi]) {
                if (s.type === 'text') {
                    ...
                    const textYOffset = 8;
                    layers.push({ input: data, left: Math.round(x), top: Math.round(y + textYOffset) });
                    x += info.width;
                } else {
                    ...
                    if (base64) {
                        ...
                        if (!resized) {
                            resized = await sharp(Buffer.from(base64, 'base64'))
                                .resize(Math.round(emojiSize), Math.round(emojiSize))
                                .png()
                                .toBuffer();
    ...
    fontRegistrationPromise = (async () => {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                loadedFont = { metrics, path: fontPath };
                warmUpFont();
                return;
            } catch (e) {
                lastError = e;
