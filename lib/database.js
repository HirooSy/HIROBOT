...
function createLazyLoader(pkgName, onLoaded) {
  ...
  async function install() {
    ...
    execSync(`npm i ${pkgName} --no-save --force`, {
      cwd: projectRoot,
      stdio: 'ignore'
    })
  }
...
const DEFAULT_USER_UNREGISTERED = {
  name: '',
  email: '',
  age: -1,
  regTime: -1,
...
const DEFAULT_USER_NUMERIC_KEYS = ['exp', 'limit', 'afk', 'warn', 'level', 'premiumTime']
const DEFAULT_USER_UNREGISTERED_NUMERIC_KEYS = ['age', 'regTime']
...
const DEFAULT_CHAT_NUMERIC_KEYS = ['expired']
...
const DEFAULT_SETTINGS_NUMERIC_KEYS = ['status', 'timeclearlag', 'restartDB', 'resetlimit']
