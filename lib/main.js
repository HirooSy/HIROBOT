...
const NAMED_TUNNEL_TOKEN = process.env.CF_TOKEN || '';
const NAMED_TUNNEL_HOSTNAME = (process.env.CF_HOSTNAME || '').replace(/^https?:\/\//, '').replace(/\/$/, '');
...
function noteIgnoredErrorAndMaybeRestart(label, msg) {
  ...
  if (now - lastForceReconnectAt < FORCE_RECONNECT_DEBOUNCE_MS) {
    console.error(`[RECONNECT DEBUG] skipped due to debounce, ${now - lastForceReconnectAt}ms since last`);
    return;
  }
  lastForceReconnectAt = now;

  console.error(`[RECONNECT DEBUG] about to resolve Connection.conn and call forceReconnect`);
  Promise.resolve(Connection.conn).then((conn) => {
    console.error(`[RECONNECT DEBUG] Connection.conn resolved: ${conn ? 'has conn object' : 'conn is falsy!'}`);
    if (!conn) return;
    ...
    console.error(`[RECONNECT DEBUG] forceReconnect resolved with: ${result}`);
...
function setupGracefulShutdown() {
  ...
  process.on('force-exit-for-restart', () => flushDatabaseAndExit(1, 'reconnect exhausted'));
}
...
function removeBrokenCloudflaredBinary(binPath) {
  if (binPath !== CLOUDFLARED_BIN_PATH) return;
  try {
...
async function pushTunnelUrlToWorkersKv(url) {
  const apiToken = process.env.CF_KV_TOKEN;
  const accountId = process.env.CF_ACCOUNT_ID;
  const namespaceId = process.env.CF_KV_NAMESPACE_ID;
  if (!apiToken || !accountId || !namespaceId) return;

  try {
    const res = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${accountId}/storage/kv/namespaces/${namespaceId}/values/current_url`,
      {
        method: 'PUT',
        headers: {
    ...
    if (!res.ok || !data?.success) {
      console.warn(chalk.yellow('Tunnel') + chalk.gray(` Failed to update Workers KV (${res.status}): ${data ? JSON.stringify(data.errors) : ''}`));
    } else {
      console.log(chalk.greenBright('Tunnel') + chalk.gray(' Workers KV updated'));
    }
  } catch (e) {
    console.warn(chalk.yellow('Tunnel') + chalk.gray(` Failed to update Workers KV: ${e.message}`));
  }
}

function getPublishedUrl(tunnelUrl) {
  if (process.env.CF_HOSTNAME) return `https://${process.env.CF_HOSTNAME}`;
  return tunnelUrl;
}
...
function setupTunnel(opts) {
  ...
  async function spawnTunnel() {
    ...
    if (saved && isPidAlive(saved.pid)) {
      ...
      opts['server'] = getPublishedUrl(saved.url);
      global.websiteState.url = getPublishedUrl(saved.url);
      lastTunnelStartAt = Date.now();
      console.log(chalk.greenBright('Tunnel ') + chalk.gray(getPublishedUrl(saved.url).replace('https://', '')));
      pushTunnelUrlToGithub(getPublishedUrl(saved.url)).catch(() => {});
      pushTunnelUrlToUptimeRobot(getPublishedUrl(saved.url)).catch(() => {});
      pushTunnelUrlToWorkersKv(saved.url).catch(() => {});
      return;
    }
    if (saved && !isPidAlive(saved.pid)) {
      clearSavedTunnel();
    ...
    try {
      ...
      opts['server'] = getPublishedUrl(url);
      global.websiteState.url = getPublishedUrl(url);
      reconnectAttempts = 0;
      isReconnecting = false;
      lastTunnelStartAt = Date.now();
      saveTunnel(url, proc.pid);
      pushTunnelUrlToGithub(getPublishedUrl(url)).catch(() => {});
      pushTunnelUrlToUptimeRobot(getPublishedUrl(url)).catch(() => {});
      pushTunnelUrlToWorkersKv(url).catch(() => {});

      console.log(chalk.greenBright('Tunnel ') + chalk.gray(getPublishedUrl(url).replace('https://', '')));
    ...
    try {
      ...
      console.log(
        ...
        chalk.gray(
          url
            ? url.replace('https://', '')
            : 'Connected (set CF_HOSTNAME di .env untuk menampilkan URL & aktifkan health check publik)'
        )
      );
...
async function main() {
  ...
  const pluginLogger = Helper.P({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({});
  ...
  if (conn.authState.creds.registered) {
    ...
    global.onBotOpen = () => setupTunnel(opts);
  }
