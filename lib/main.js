...
const sys = global.settings.system || {};
global.opts = {
  ...Helper.opts,
  queue:    Helper.opts['queue']    ?? !!sys.queue,
  autoread: Helper.opts['autoread'] ?? !!sys.autoRead,
  restrict: Helper.opts['restrict'] ?? true,
  self:     Helper.opts['self']     ?? false,
  nyimak:   Helper.opts['nyimak']   ?? false,
  pconly:   Helper.opts['pconly']   ?? sys.OnlyRespondTo === 'dm',
  gconly:   Helper.opts['gconly']   ?? sys.OnlyRespondTo === 'group',
  swonly:   Helper.opts['swonly']   ?? sys.OnlyRespondTo === 'status',
};
global.prefix = Array.isArray(sys.prefix) && sys.prefix.length ? sys.prefix : ['.', '/', '!'];
...
const NO_RELOAD_FILES = new Set(['utils/main.js', 'utils/start.js'])
...
async function main() {
  ...
  Object.assign(global, { ...Helper, timestamp: { start: Date.now() } });
