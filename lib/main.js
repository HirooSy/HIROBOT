...
function cleanupConnectionIntervals() {
  const kept = [memMonitorInterval, refillInterval, writeInterval, clearStoreInterval, tmpClearInterval];
...
function clearDataStore() {
  try {
    if (Connection.store?.pruneChats) Connection.store.pruneChats(7 * 24 * 60 * 60 * 1000);
...
function startDataStoreCleanup() {
  clearDataStore();
  clearStoreInterval = setInterval(clearDataStore, 30 * 60 * 1000);
...
function watchLibFile(libDir, file) {
  fs.watchFile(file, async () => {
    ...
    libReloadTimers[file] = setTimeout(async () => {
      ...
      console.log(chalk.greenBright(` [ Update '${rel}' ]`));
      try {
        await reimportLibFile(file, rel)
        console.log(chalk.greenBright(` [ Update '${rel}' ] reloaded`));
      } catch (e) {
        console.error(chalk.red(` [ Update '${rel}' ] reload gagal:`), e.message)
      }
