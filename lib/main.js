...
const BAILEYS_STACK_MARKERS = ['/baileys/', '@whiskeysockets/baileys', 'undici', '/baileys-lite/'];
const IGNORED_ERROR_WINDOW_MS = 120 * 1000; // TEMP: diperpanjang buat debug, balikin ke 30*1000 kalau udah kelar
...
const IGNORED_ERROR_THRESHOLD = 10; // TEMP: diperlonggar buat debug, balikin ke 3 kalau udah kelar
...
function isBaileysInternalError(err) {
  ...
  return BAILEYS_STACK_MARKERS.some(marker => stack.includes(marker));
}
...
async function main() {
  ...
  Connection.onConnectionUpdate(({ connection }) => {
