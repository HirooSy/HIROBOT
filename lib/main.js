...
const IGNORED_ERROR_WINDOW_MS = 30 * 1000;
const FORCE_RECONNECT_DEBOUNCE_MS = 5 * 1000;
...
const IGNORED_ERROR_THRESHOLD = 3;
...
function noteIgnoredErrorAndMaybeRestart(label, msg) {
  ...
  // These internal errors happen deep inside Baileys (decrypt/noise-handler/
  // socket code) and never produce a real 'connection.update' close event,
  // so connectionUpdate()'s own reconnect logic never fires and the socket
  // is left dangling until the watchdog threshold above is eventually hit.
  // Instead of waiting for that, proactively force a reconnect as soon as
  // one of these is seen, debounced so a burst of errors only triggers one
  // reload attempt instead of stacking several.
  if (now - lastForceReconnectAt < FORCE_RECONNECT_DEBOUNCE_MS) return;
  lastForceReconnectAt = now;

  Promise.resolve(Connection.conn).then((conn) => {
    if (!conn) return;
    return Connection.forceReconnect(conn, {
      store: Connection.store,
      authState: Connection.authState,
      logger: Connection.logger,
    }, 'baileys internal error');
  }).catch((err) => console.error(chalk.red('Forced reconnect error:'), err?.message || err));
}
...
async function flushDatabaseAndExit(code, signalName) {
  ...
  // 5s was too tight for MongoDB/MySQL writes that touch many collections
  // sequentially (now parallelized, see database.js), especially right
  // after a reconnect when the DB connection itself may still be settling.
  // Bumped to 12s, and on timeout we give the in-flight write a second,
  // shorter window instead of abandoning it immediately - the original
  // write() call is still running in the background (Promise.race doesn't
  // cancel it), so if it was only slightly late this avoids losing data
  // that would have been saved a moment later anyway.
  const write = () => (global.db?.data ? global.db.write() : Promise.resolve());
  ...
  try {
    await withTimeout(pendingWrite, 12000);
  } catch (err) {
    console.error(chalk.red('[ Shutdown ]'), 'Database save slow, giving it one more moment:', err.message);
    try {
      await withTimeout(pendingWrite, 5000);
    } catch (err2) {
      console.error(chalk.red('[ Shutdown ]'), 'Failed to save database before exit:', err2.message);
    }
  } finally {
    process.exit(code);
  }
...
// If a spawn attempt against the cached binary fails in a way that
// indicates the file itself is bad (wrong architecture, truncated
// download, not actually executable) rather than a transient runtime
// issue, delete it so ensureCloudflaredBinary() re-downloads a fresh copy
// on the next attempt instead of retrying against the same broken file
// forever.
function removeBrokenCloudflaredBinary(binPath) {
  if (binPath !== CLOUDFLARED_BIN_PATH) return; // never delete a system-PATH 'cloudflared'
  try {
    if (fs.existsSync(binPath)) {
      fs.unlinkSync(binPath);
      console.warn(chalk.yellow('Tunnel') + chalk.gray(' Removed cloudflared binary after spawn failure, will re-download next attempt.'));
    }
  } catch (e) {
    console.error(chalk.red('Tunnel') + chalk.gray(` Failed to remove broken cloudflared binary: ${e.message}`));
  }
}
...
function setupTunnel(opts) {
  ...
  async function spawnTunnel() {
    ...
    try {
      ...
      let proc;
      try {
        proc = spawn(cloudflaredBin, [
          'tunnel',
          // Use 127.0.0.1 explicitly rather than 'localhost': some
          // containers resolve 'localhost' to the IPv6 loopback (::1)
          // first, and if the HTTP server only bound to the IPv4 wildcard
          // (0.0.0.0, which does NOT cover ::1), cloudflared gets
          // "connection refused" trying to reach it even though the
          // tunnel itself connected to Cloudflare's edge fine - the quick
          // tunnel URL comes up, but every request 502s.
          '--url', `http://127.0.0.1:${PORT}`,
          '--no-autoupdate',
        ], { detached: true, stdio: ['pipe', 'pipe', 'pipe'] });
      } catch (spawnErr) {
        // spawn() itself throws synchronously for some failure modes
        // (e.g. EACCES). If the binary we downloaded is corrupt or the
        // wrong architecture for this container, retrying with the exact
        // same broken file forever would never recover - remove it so the
        // next attempt re-downloads from scratch.
        removeBrokenCloudflaredBinary(cloudflaredBin);
        throw spawnErr;
      }
      let resolved = false;
      // Cloudflared's actual failure reason (permission denied, exec format
      // error, no route to trycloudflare.com, etc.) is only ever printed to
      // its own stdout/stderr - previously we only surfaced a bare exit
      // code on failure and discarded that output entirely, so environments
      // where cloudflared can't run at all (e.g. some restricted container
      // panels) just logged "exited early with code 1" with no way to tell
      // why. Keep a rolling tail of its output so we can log it.
      const outputTail = [];
      ...
      const captureOutput = (chunk) => {
        ...
        for (const line of text.split('\n')) {
          ...
          if (outputTail.length > MAX_TAIL_LINES) outputTail.shift();
        }
        return text;
      };
        ...
        proc.once('exit', (code) => {
          if (!resolved) {
            const tail = outputTail.length
              ? `\n${outputTail.join('\n')}`
              : ' (no output captured - likely failed to even start; check binary architecture/permissions)';
            reject(new Error(`cloudflared exited early with code ${code}:${tail}`));
          }
        });
        setTimeout(() => {
          if (!resolved) {
            const tail = outputTail.length ? `\n${outputTail.join('\n')}` : '';
            reject(new Error(`Timed out waiting for tunnel URL${tail}`));
          }
        }, 25000);
      });
