...
import { plugins, loadPluginFiles, pluginFolder, pluginFilter } from './utils/plugins.js'
import chalk from './utils/color.js'
import fs from 'fs';
import { tmpdir } from 'os'

// opts & prefix resolution now lives in helper.js (single source of truth,
// reads directly from global.settings.system in config.js). global.opts stays
// a plain mutable object here (not a getter) because plugins like
// plugins/main/enable.js toggle it at runtime, e.g. global.opts['autoread'] = true.
...
// Flush any pending in-memory database changes to disk before the process
// exits. Covers cases that don't already call db.write() themselves: panel
// restarts/stops (SIGTERM/SIGINT), and the internal watchdog/memory-limit
// process.exit() calls below. Guarded so it only ever runs once and won't
// hang shutdown if the write itself fails or takes too long.
...
const EXCLUDED_WATCH_DIRS = new Set(['node_modules', '.git', 'data', '.cache'])

function collectLibJsFiles(dir) {
  let results = [];
  const entries = fs.readdirSync(dir, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory() && EXCLUDED_WATCH_DIRS.has(entry.name)) continue;
    const full = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      results = results.concat(collectLibJsFiles(full));
...
const libReloadTimers = {}
const LIB_RELOAD_DEBOUNCE_MS = 500
// NOTE: main.js and start.js live directly under lib/, so path.relative(libDir, file)
// resolves to "main.js" / "start.js" — not "utils/main.js" / "utils/start.js".
// The old (wrong) paths never matched, so main.js's self-reload was never skipped,
// causing main() to re-run on every edit and re-bind the HTTP server -> EADDRINUSE.
const NO_RELOAD_FILES = new Set(['main.js', 'start.js'])

async function reimportLibFile(file, rel) {
  const mod = await Helper.importFile(file)

  if (rel === 'utils/simple.js') {
    const { protoType, serialize } = mod
    if (typeof protoType === 'function') protoType()
    if (typeof serialize === 'function') serialize()
    const conn = await Connection.conn
    if (conn && typeof mod.HelperConnection === 'function') mod.HelperConnection(conn, { store: Connection.store, logger: conn.logger })
  }
...
function watchLibFile(libDir, file) {
  fs.watchFile(file, async () => {
    if (libReloadTimers[file]) {
      clearTimeout(libReloadTimers[file])
    }
    libReloadTimers[file] = setTimeout(async () => {
      delete libReloadTimers[file]
      const rel = path.relative(libDir, file);
      if (NO_RELOAD_FILES.has(rel)) return
      try {
        await reimportLibFile(file, rel)
        console.log(chalk.greenBright(`Reloaded — [${rel}]`));
      } catch (e) {
        console.error(chalk.red(`Reload Failed — [${rel}]:`), e.message)
      }
    }, LIB_RELOAD_DEBOUNCE_MS)
  });
}

function watchLibFiles() {
  const libDir = Helper.__dirname(import.meta.url);
  const files = collectLibJsFiles(libDir);
  for (const file of files) watchLibFile(libDir, file);
}

async function loadPlugins() {
  const logger = P({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({});
  await loadPluginFiles(pluginFolder, pluginFilter, { logger, recursiveRead: true }).catch(console.error);
}
...
async function main() {
  ...
  // NOTE: Helper's default export also carries `opts`/`prefix`/`prefixList`/`runtimeOpts`
  // (its own raw CLI-parsed values). A plain `{ ...Helper }` spread here would clobber
  // the correctly-merged global.opts / global.prefix set at module load time. Exclude them.
