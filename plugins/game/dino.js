function buildGameHtml(scoreToken, apiBase) {
  const httpsApiBase = apiBase ? apiBase.replace(/^http:\/\//, 'https://') : '';
  return `<style>*{-webkit-tap-highlight-color:transparent;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none;box-sizing:border-box}</style>
...
<div style="width:100%;max-width:620px;margin:auto;padding:16px">
...
<div><div style="font-size:11px;letter-spacing:1.5px;color:rgba(255,255,255,.45)">DINO RUNNER</div><div style="font-size:21px;font-weight:bold;color:#fff">Endless Run</div></div>
...
function connectWs(){
  ...
  ws.onmessage=(e)=>{let msg;try{msg=JSON.parse(e.data)}catch{return}
    if(msg.type==='aiRichActionResult'&&pending.has(msg.requestId)){pending.get(msg.requestId)(msg);pending.delete(msg.requestId)}
  };
...
function loadBest(){let v=0;try{const s=localStorage.getItem('dino_best');if(s)v=parseInt(s,10)}catch(e){}return isNaN(v)?0:v}
...
let handler = async (m, { conn }) => {
    ...
    const scoreToken = global.registerHtmlAction({
        ...
        run: async (conn, chatId, args) => {
            const finalScore = Math.max(0, Math.floor(Number(args?.score) || 0));
            const gemsEarned = Math.floor(finalScore / 1000);

            if (gemsEarned > 0) {
                if (!global.db.data.users[m.sender]) {
                    throw new Error('User not registered.');
                }
                global.db.data.users[m.sender].gems = (global.db.data.users[m.sender].gems || 0) + gemsEarned;
                await global.db.write();
            }

            return { gems: gemsEarned, score: finalScore };
        }
    });

    rich.addHtml(buildGameHtml(scoreToken, apiBase), { trustedSources });

    await rich.send(m.chat, { quoted: m });
};

handler.help = handler.command = ['dino', 'dinorunner'];
handler.tags = ['game'];
handler.limit = false;
handler.ai = { risk: 'low', description: 'play the dino runner game and earn gems based on score' };
