...
async function getConfig() {
    ...
    const client   = findTag(xml, 'client')
    const dl       = findTag(xml, 'download')
    const ul       = findTag(xml, 'upload')
    const srvCfg   = findTag(xml, 'server-config')
...
async function getServers(config) {
    ...
    for (const baseUrl of urls) {
        try {
            ...
            const elements = findAllTags(xml, 'server')
