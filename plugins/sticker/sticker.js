...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  let wmSticker = JSON.parse(process.env.STICKER_WM || '["", ""]')
  let scap = {
...
async function addExif(buffer, packname, author, categories = [''], extra = {}) {
	const img = new Image()
	const json = { 'sticker-pack-id': process.env.BOT_NAME, 'sticker-pack-name': packname, 'sticker-pack-publisher': author, 'emojis': categories, 'is-ai-sticker': 1, ...extra }
